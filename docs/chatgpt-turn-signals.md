...
- **`node.closest(OWN_SURFACES)`** — a interface da própria extensão registrava
  a confirmação de conexão do conector Desktop como falha do ChatGPT.
...
console's execution context from the page to this extension's content script first; otherwise

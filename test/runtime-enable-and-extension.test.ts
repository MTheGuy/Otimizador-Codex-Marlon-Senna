...
describe('companion extension setup contract', () => {
  it('keeps standalone recovery visible without pointing an installed app at releases/latest', async () => {
    ...
    expect(html).toMatch(/id="bridgeDownload"[\s\S]*?Baixar ZIP da extensão/i);
    expect(html).toMatch(/Necessário para subagentes/i);
    expect(html).toMatch(/Exige a extensão do Chrome carregada e conectada/i);

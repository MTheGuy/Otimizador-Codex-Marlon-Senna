...
beforeAll(async () => {
  ...
  const brandingSource = await fs.readFile(path.join(process.cwd(), 'extension', 'branding.js'), 'utf8');
  backgroundSource = brandingSource + '\n' + backgroundSource.replace("import './branding.js';", '').replace(/\r\n/g, '\n');
});
...
describe('extension release metadata', () => {
  ...
  it('runs only the fiber and bounded usage readers in the page context', async () => {
    ...
    expect(main.flatMap((entry) => entry.js).sort()).toEqual(['branding.js', 'branding.js', 'fiber.js', 'usage.js']);
...
function response(status: number, data: unknown) {
  const body =
    data && typeof data === 'object' && (data as Record<string, unknown>).app === BRANDING.appId
    ...
    it(`closes only the exact accepted helper document (${outcome})`, async () => {
      ...
      const worker = loadWorker({
        ...
        fetch: async (input) => {
          ...
          if (route === '/hello') return response(200, { app: BRANDING.appId, paired: true });
...
describe('exact chat recovery from a fresh Chrome tab scan', () => {
  ...
  function appWith(repair: string | null) {
    ...
    const fetch = vi.fn(async (input: string) => {
      ...
      if (url.pathname === '/hello') return response(200, { app: BRANDING.appId, paired: true });
...
describe('app-owned retained tab pool', () => {
  ...
  async function budget(options: { safe?: (tab: number) => boolean; changed?: number; keep?: number; recent?: number; protectDuplicate?: boolean; reverseActivity?: boolean } = {}) {
    ...
    const worker = loadWorker({
      ...
      session: new FakeStorageArea({ tabDocuments: Object.fromEntries(tabs.map(tab => [tab.id, `doc-${tab.id}`])), tabEpochs: Object.fromEntries(tabs.map(tab => [tab.id, 0])) }),
      fetch: async input => response(200, new URL(input).pathname === '/hello' ? { app: BRANDING.appId, paired: true } : {
...
describe('extension command delivery', () => {
  ...
  it('re-injects the recorder into already-open ChatGPT tabs after an extension reload', async () => {
    ...
    expect(worker.scriptingExecuteScript.mock.calls).toEqual([
      [{ target: { tabId: 41 }, files: ['branding.js', 'chatgpt-dom.js'] }],
      [{ target: { tabId: 41 }, world: 'MAIN', files: ['branding.js', 'fiber.js'] }],
      [{ target: { tabId: 41 }, files: ['branding.js', 'content.js'] }],
      [{ target: { tabId: 42 }, files: ['branding.js', 'chatgpt-dom.js'] }],
      [{ target: { tabId: 42 }, world: 'MAIN', files: ['branding.js', 'fiber.js'] }],
      [{ target: { tabId: 42 }, files: ['branding.js', 'content.js'] }]
    ]);
  ...
  it('provisions itself silently on the first call and retries with the new token', async () => {
    ...
    const fetch = vi.fn(async (input: string, init: Record<string, unknown> = {}) => {
      ...
      if (url.pathname === '/hello') return response(200, { app: BRANDING.appId, paired: false });
  ...
  function app(): {
    ...
    const state = {
      ...
      async fetch(input: string) {
        ...
        if (input.endsWith('/hello')) return response(200, { app: BRANDING.appId, paired: true });
  ...
  it('mirrors an app-side disconnect from hello before the popup can report a stale pair', async () => {
    ...
    const fetch = vi.fn(async (input: string) => {
      ...
      if (url.pathname === '/hello') {
        return response(200, { app: BRANDING.appId, paired: false, disconnected: true });
      }
      if (url.pathname === '/pair') {
        pairCalls++;

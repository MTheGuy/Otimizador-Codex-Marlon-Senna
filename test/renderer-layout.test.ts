...
it('searches whole settings sections without empty headings, orphaned controls or lost conditional visibility', () => {
  ...
  filterSettingsSections(view, '  ENCERRAMENTO DA SESSÃO  ');
  expect(sections.filter(section => !section.hidden).map(section => section.textContent)).toEqual(['Manter o turno aberto']);
...
it('exposes Goal tool context as an opt-in setting wired into the existing form', () => {
  ...
  expect(toggle.closest('label')?.textContent).toContain('argumentos e resultados registrados');
...
describe('the session-row chat actions', () => {
  it('keeps current-chat pressure in the conversation detail', () => {
    ...
    expect(chatSource).toContain('tokens estimados no contexto atual');
  });
   ...
   */
  it('mantém chamadas sem atribuição bloqueadas e sem ação que conceda autoridade', () => {
    expect(chatSource).toMatch(/function unattributedBlocked\(\): boolean\s*\{\s*return true;/);
    expect(chatSource).not.toContain('toggleUnattributedBlock');
    expect(chatSource).toMatch(/const actions: HTMLButtonElement\[\] = \[\];[\s\S]{0,100}if \(summary\.conversationId === null\)[\s\S]{0,400}return row;/);
    const toggle = document.getElementById('allowUnattributedCalls') as HTMLInputElement;
    expect(toggle.hidden).toBe(true);
    expect(toggle.disabled).toBe(true);
    // Same word and same tone as a blocked chat: one state, read the same way down the list.
    expect(chatSource).toMatch(/unattributedBlocked\(\)[\s\S]{0,120}text: 'bloqueado', tone: 'is-failed'/);
  ...
  it('saves every field it shows', () => {
    ...
    for (const input of pane.querySelectorAll<HTMLInputElement>('.pane input')) {
      if (input.id === 'displayName') {
        expect(mainSource).toContain("$('displayName').addEventListener('change', () => void save())");
        expect(mainSource).toContain("displayName: $<HTMLInputElement>('displayName').value.trim()");
        continue;
      }

...
const canonicalRepository = BRANDING.publication.repository;
const [maintainerLogin, repositoryName] = canonicalRepository.split('/');
const safeEmail = `000000+${maintainerLogin}@users.noreply.github.com`;
const privateEmail = `${maintainerLogin}@example.invalid`;
...
function commit(repository: string, message: string, email: string): void {
  execFileSync('git', ['commit', '--allow-empty', '-m', message], {
    ...
    env: {
      ...process.env,
      GIT_AUTHOR_NAME: BRANDING.author,
      GIT_AUTHOR_EMAIL: email,
      GIT_COMMITTER_NAME: BRANDING.author,
...
describe('public-history privacy gate', () => {
  ...
  it.each([
    ...
    `git@github.com:${canonicalRepository}.git`,
    `ssh://git@github.com/${canonicalRepository}`
    ...
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    commit(repository, 'Already public canonical commit', privateEmail);
    ...
    commit(repository, 'New unpublished unsafe identity', privateEmail);
    expect(verify(repository).status).toBe(1);
  });

  it.each([
    `https://github.com/example/${repositoryName}.git`,
    `https://github.com/${canonicalRepository}-extra.git`,
    `https://github.com.example/${canonicalRepository}.git`
  ])('does not trust an unrelated upstream URL (%s)', (url) => {
    const repository = makeRepository();
    execFileSync('git', ['remote', 'add', 'upstream', url], { cwd: repository });
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    commit(repository, 'Unpublished identity', privateEmail);
  ...
  it('does not fall back to fork history when canonical main has not been fetched', () => {
    const repository = makeRepository();
    execFileSync('git', ['remote', 'add', 'upstream', `https://github.com/${canonicalRepository}.git`], { cwd: repository });
    commit(repository, 'Only published on a fork', privateEmail);
    execFileSync('git', ['update-ref', 'refs/remotes/origin/main', 'HEAD'], { cwd: repository });
    expect(verify(repository).status).toBe(1);
  });

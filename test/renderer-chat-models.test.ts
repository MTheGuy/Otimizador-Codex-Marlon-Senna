...
it('renders the two observed Pro generations separately and sends their exact selection identities', async () => {
  ...
  expect(dom.window.document.getElementById('contextMeterInfo')!.textContent).toContain('Compactação automática desativada no Pro');
  ...
  expect(dom.window.document.getElementById('contextMeterInfo')!.textContent).toMatch(/Compactação automática em 400[,.]000 tokens/);
});
...
it('offers only observed models, prefers supported GPT-6 High, and replaces a removed account selection', async () => {
  ...
  expect(reload.getAttribute('aria-label')).toBe('Recarregar modelos do ChatGPT');
  const slider = dom.window.document.querySelector<HTMLInputElement>('#composerPowerChoices input')!;
  expect(slider.max).toBe('2');
  expect(slider.getAttribute('aria-valuetext')).toBe('GPT-6 \u00b7 Alto');
  slider.value = '0'; slider.dispatchEvent(new dom.window.Event('input'));
...
it('limits the stepped composer to observed Sol Low upwards and Astra without inventing Pro access', async () => {
  ...
  for (const [index, [model, reasoningEffort]] of expected.entries()) {
    ...
    expect(slider.getAttribute('aria-valuetext')).not.toMatch(/Instantâneo|5\.5|Pro|Mínimo/);
  }
...
it('keeps the trigger consistent with send admission during reload and a removed effort', async () => {
  ...
  expect(label.textContent).toBe('GPT-5.6 Sol · Alto');
  ...
  expect(confirmedComposerModel()).toBeNull();
  expect(label.textContent).toBe('Carregando modelos…');

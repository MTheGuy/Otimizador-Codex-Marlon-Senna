...
const brandingSource = readFileSync(new URL('../extension/branding.js', import.meta.url), 'utf8');
// A VM recebe os mesmos arquivos da extensão, com a dependência de módulo já carregada.
const source = `${brandingSource}\n${backgroundSource.replace("import './branding.js';", '')}`;
...
async function worker(inputs: Array<{ id: string; conversationId: string | null }>, modelCatalogRequest?: { nonce: string; expiresAt: number }, priorLocal: Record<string, unknown> = {}) {
  ...
  const fetch = vi.fn(async (input: string, _init?: RequestInit): Promise<{ ok: boolean; status: number; json: () => Promise<unknown> }> => ({
    ...
    json: async () => new URL(input).pathname === '/hello'
      ? { app: BRANDING.appId, bridge: BRIDGE_PROTOCOL, compatible: true, paired: true }
  ...
  const context = vm.createContext({
    chrome: {
      ...
      windows,
      runtime: { getManifest: () => ({ version: BRANDING.version }), onMessage: event, onInstalled: event, onStartup: event },
...
describe('one browser maintenance flight per desktop outbox publication', () => {
  ...
  it('sizes the owned minimized window from desktop work area without requesting focus or restore', async () => {
    ...
    h.fetch.mockResolvedValue({ ok: true, status: 200, json: async () => ({ app: BRANDING.appId, bridge: BRIDGE_PROTOCOL, compatible: true, paired: true, ok: true, inputs: [{ id: firstId, conversationId: null }], background: true, browserWorkArea: { x: -1920, y: 0, width: 1920, height: 1040 } }) });
    await h.maintain();
  ...
  it('places an offered worker in its unfocused background window with discard protection', async () => {
    ...
    h.fetch.mockImplementation(async (input) => ({ ok: true, status: 200, json: async () => {
      if (new URL(input).pathname === '/hello') return { app: BRANDING.appId, bridge: BRIDGE_PROTOCOL, compatible: true, paired: true };

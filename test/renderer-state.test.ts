...
import { securityPreset } from '../src/shared/security-profiles.js';
...
it('does not overwrite a focused dirty settings field on an unsolicited state push', async () => {
  ...
  expect(allowUnattributed.hidden).toBe(true);
  expect(allowUnattributed.disabled).toBe(true);
  expect(allowUnattributed.checked).toBe(false);
  ...
  expect(w.document.getElementById('facts')!.textContent).toContain('Ferramentas do conector principal e computador3 total');
  ...
  expect(accessWarning.textContent).toContain('Acessibilidade: missing');
  expect(accessWarning.textContent).toContain('Resultado atual do componente nativo');
...
it('preserves native Desktop permissions when saving unrelated settings on Linux', async () => {
  ...
  for (const group of ['screen', 'control', 'clipboardRead', 'clipboardWrite']) {
    expect(w.document.querySelector<HTMLElement>(`[data-group="${group}"]`)!.hidden).toBe(true);
  }
...
it('uses native menu-bar/Dock wording on macOS instead of Windows tray copy', async () => {
  ...
  expect(doc.getElementById('backgroundRunningCopy')!.textContent).toContain('barra de menus e no Dock');
  expect(doc.getElementById('backgroundRunningCopy')!.textContent).not.toContain('tray');
  expect(doc.getElementById('minimizeToTrayCopy')!.textContent).toBe('Ocultar na barra de menus ao fechar a janela');
});
...
it('surfaces the existing root rename API in the folder row', async () => {
  ...
  const doc = mounted.window.document;
  const button = doc.querySelector<HTMLButtonElement>('.root button[title="Renomear /repo"]');
...
it('preserves an in-progress root rename across unrelated state pushes and cancels it if the root disappears', async () => {
  ...
  const doc = mounted.window.document;
  doc.querySelector<HTMLButtonElement>('.root button[title="Renomear /repo"]')!.click();
...
it('guides rootless setup from the capabilities that actually need a filesystem root', async () => {
  ...
  expect(connect.title).toContain('Escolha um projeto');
...
it('requires a live browser only when a browser-backed feature is actually enabled', async () => {
  ...
  expect(doc.getElementById('bridgeState')!.textContent).toContain('Autorizada');
  expect(doc.getElementById('bridgeState')!.textContent).not.toContain('Conectado em');
  ...
  expect(doc.getElementById('bridgeState')!.textContent).toContain('Conectado em');
  ...
  expect(doc.getElementById('wizard')!.classList.contains('is-tidy')).toBe(true);
  expect(doc.getElementById('bridgeState')!.textContent).toContain('não é necessária');
  expect((doc.getElementById('chatAutomation') as HTMLSelectElement).disabled).toBe(true);
  expect(doc.getElementById('chatAutomation')!.title).toMatch(/gravação/i);
});

it('não apresenta atualização automática nem reage a metadados legados', async () => {
  const mounted = await mountChat();
  const legacy = structuredClone(mounted.state);
  legacy.update = { current: '1.0.0', latest: '9.0.0', stage: 'ready', checkedAt: Date.now() };
  mounted.push(legacy);
  for (const id of ['updateLine', 'updateNotice', 'updateGet', 'updateInstall', 'installUpdate']) {
    expect(mounted.window.document.getElementById(id)).toBeNull();
  }
  expect(mounted.window.document.querySelector('.toast')).toBeNull();
});
...
it('reports stored API credentials without exposing app-wide Goal switches', async () => {
  ...
  await settle();
  expect(mounted.window.document.getElementById('goalKeyState')!.textContent).toContain('Há uma chave salva');
...
it('keeps the model in use when OpenRouter cannot be reached', async () => {
  ...
  (doc.getElementById('goalPick') as HTMLButtonElement).click();
  await settle();
  expect(doc.getElementById('goalModelsState')!.textContent).toContain('mantido');
...
it('aplica perfis somente após ação explícita e mantém a proteção de chamadas sem atribuição', async () => {
  const mounted = await mountChat();
  mounted.state.config = { ...mounted.state.config, ...securityPreset('strict'), roots: [], security: { reviewed: false } };
  mounted.push(structuredClone(mounted.state));
  const doc = mounted.window.document;
  const profile = doc.getElementById('securityProfile') as HTMLSelectElement;
  expect(profile.value).toBe('strict');
  expect(doc.getElementById('securityReview')!.hidden).toBe(false);
  expect(mounted.calls).toHaveLength(0);
  (doc.getElementById('securityConfirm') as HTMLButtonElement).click();
  await vi.waitFor(() => expect(mounted.state.config.security.reviewed).toBe(true));
  expect(mounted.state.config).toMatchObject(securityPreset('strict')!);
  expect(doc.getElementById('securityReview')!.hidden).toBe(true);
  profile.value = 'balanced';
  profile.dispatchEvent(new mounted.window.Event('change', { bubbles: true }));
  expect(mounted.state.config.readOnly).toBe(true);
  (doc.getElementById('securityApply') as HTMLButtonElement).click();
  await vi.waitFor(() => expect(mounted.state.config.readOnly).toBe(false));
  expect(mounted.state.config).toMatchObject(securityPreset('balanced')!);
  const priorCalls = mounted.calls.length;
  profile.value = 'custom';
  profile.dispatchEvent(new mounted.window.Event('change', { bubbles: true }));
  (doc.getElementById('securityApply') as HTMLButtonElement).click();
  expect(mounted.calls).toHaveLength(priorCalls);
  const read = doc.querySelector<HTMLInputElement>('[data-cap="read"]')!;
  read.checked = false;
  read.dispatchEvent(new mounted.window.Event('change', { bubbles: true }));
  await vi.waitFor(() => expect(mounted.state.config.capabilities.read).toBe(false));
  expect(profile.value).toBe('custom');
  profile.value = 'strict';
  profile.dispatchEvent(new mounted.window.Event('change', { bubbles: true }));
  (doc.getElementById('securityApply') as HTMLButtonElement).click();
  await vi.waitFor(() => expect(mounted.state.config.readOnly).toBe(true));
  expect(mounted.state.config).toMatchObject(securityPreset('strict')!);
  expect(mounted.state.config.roots).toEqual([]);
  expect(mounted.calls.every(patch => patch.multiAgent.allowUnattributedCalls === false)).toBe(true);
});

it('salva e restaura o nome exibido sem alterar permissões ou projetos', async () => {
  const mounted = await mountChat();
  const doc = mounted.window.document;
  const field = doc.getElementById('displayName') as HTMLInputElement;
  const capabilities = structuredClone(mounted.state.config.capabilities);
  const roots = structuredClone(mounted.state.config.roots);
  expect(field.value).toBe(BRANDING.name);
  field.value = 'Meu painel local';
  field.dispatchEvent(new mounted.window.Event('change', { bubbles: true }));
  await vi.waitFor(() => expect(doc.title).toBe('Meu painel local'));
  expect(mounted.state.config.ui.displayName).toBe('Meu painel local');
  expect(doc.querySelector('[data-app-name]')?.textContent).toBe('Meu painel local');
  (doc.getElementById('resetDisplayName') as HTMLButtonElement).click();
  await vi.waitFor(() => expect(doc.title).toBe(BRANDING.name));
  expect(field.value).toBe(BRANDING.name);
  expect(mounted.state.config.capabilities).toEqual(capabilities);
  expect(mounted.state.config.roots).toEqual(roots);
});

...
it('distinguishes worker status, messages and final reports', () => {
  ...
  expect(title('[worker-1 is awake again] It resumed')).toBe('worker-1 retomou o trabalho');
  expect(title('[worker-1 reported] RESULT: Done')).toBe('worker-1 concluiu · relatório');
  expect(title('The room is ready for review')).toBe('Mensagem de worker-1');
});

...
it('keeps Pro static and identifies token estimates and compaction exclusion', () => {
  ...
  expect(doc.getElementById('contextMeterInfo')?.textContent).toContain('Compactação automática desativada no Pro');
  expect(doc.getElementById('contextMeterInfo')?.textContent).toContain('estimativa');
});
it('uses configured limits for ordinary models and supports click and Escape', () => {
  const doc = setup('gpt-5.6-sol-high');
  expect(doc.getElementById('contextMeterInfo')?.textContent).toContain('50% do limite configurado');
  initContextMeter();

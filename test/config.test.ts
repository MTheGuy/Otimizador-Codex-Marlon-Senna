...
describe('shipped defaults', () => {
  it('exibe a marca configurada por padrão sem exigir personalização manual', () => {
    expect(defaultConfig().ui.displayName).toBe(BRANDING.name);
  });

  it('salva apenas o nome humano personalizado e restaura o padrão com campo vazio', async () => {
    const config = defaultConfig();
    await saveConfig({ ...config, ui: { ...config.ui, displayName: '  Meu assistente local  ' } });
    expect((await loadConfig()).ui.displayName).toBe('Meu assistente local');
    await saveConfig({ ...config, ui: { ...config.ui, displayName: '' } });
    expect((await loadConfig()).ui.displayName).toBe(BRANDING.name);
  });

  it('um nome inválido não descarta as demais configurações', async () => {
    const config = defaultConfig();
    const loaded = await saveConfig({ ...config, readOnly: false, ui: { ...config.ui, displayName: 'x'.repeat(121) } });
    expect(loaded.ui.displayName).toBe(BRANDING.name);
  ...
  it('inicia sem projetos aprovados e com permissões STRICT', async () => {
    await fs.rm(path.join(dir, 'config.json'), { force: true });
    const loaded = await loadConfig();
    expect(loaded.readOnly).toBe(true);
    expect(loaded.roots).toEqual([]);
    expect(loaded.security).toEqual({ reviewed: false });
  ...
  it.each(['win32', 'darwin', 'linux'] as const)(
    'mantém STRICT sem comandos ou desktop por padrão em %s',
    (platform) => {
  ...
  it('não permite reabrir chamadas sem atribuição pela configuração', async () => {
    const config = defaultConfig();
    expect(config.multiAgent.allowUnattributedCalls).toBe(false);
  ...
  it('preserva a revisão explícita sem conceder permissões adicionais', async () => {
    const config = defaultConfig();
    await saveConfig({ ...config, security: { reviewed: true } });
    const loaded = await loadConfig();
    expect(loaded.security?.reviewed).toBe(true);
    expect(loaded.capabilities).toEqual(DEFAULT_CAPABILITIES);
    expect(loaded.readOnly).toBe(true);
    expect(loaded.roots).toEqual([]);
  });

  it('repara a opção legada insegura sem apagar projetos ou permissões escolhidas', async () => {
    const config = defaultConfig();
    const roots = [{ name: 'projeto', path: 'C:\\Users\\example\\project' }];
    await fs.writeFile(path.join(dir, 'config.json'), JSON.stringify({
      ...config, roots, readOnly: false,
      capabilities: { ...config.capabilities, edit: true },
      multiAgent: { ...config.multiAgent, allowUnattributedCalls: true }
    }), 'utf8');
    const loaded = await loadConfig();
    expect(loaded.multiAgent.allowUnattributedCalls).toBe(false);
    expect(loaded.roots).toEqual(roots);
    expect(loaded.capabilities.edit).toBe(true);
    expect(loaded.readOnly).toBe(false);
  });
});

...
import { BRANDING } from '../src/shared/branding.js';
...
beforeAll(async () => {
  ...
  contentSource = await fs.readFile(path.join(process.cwd(), 'extension', 'branding.js'), 'utf8') + '\n' + contentSource;
});
...
describe('the calls a row folded away', () => {
  const FOLDED = {
    ...
    index: 0,
    tool: 'run_command',
    path: '/' + BRANDING.name + ' Core/mcp/run_command',
    app: BRANDING.name + ' Core',
    resource: null,
    messageId: 'msg-1',
    turnId: 'turn-1',
...
describe('canonical Fiber transcript ingestion in 1.8', () => {
  ...
  it('drops an oversized rendered capture instead of publishing cut markup', async () => {
    ...
    expect(wideEvent.text).toContain('Observação do navegador reduzida');
...
describe('the app-owned chronological stream', () => {
  ...
  it('shows local calls even when ChatGPT rendered no native tool row for them', async () => {
    ...
    expect(rows).toEqual(['Turno iniciado', 'Checking the repository', 'Read second.ts', 'Read third.ts']);
  ...
  it('ignores ChatGPT DOM reasoning order and renders only the order recorded by the app', async () => {
    ...
    expect(rows).toEqual(['Turno iniciado', 'Listed approved folders', 'Listed open windows']);
    expect(section.getAttribute('data-clf-turn-replaced')).toBe('1');
...
describe('where the page stream puts an event that was recorded late', () => {
  ...
  it('moves a late arrival into its slot instead of appending it to the feed', async () => {
    ...
    expect(before).toEqual(['Turno iniciado', 'Checking the repository', 'Writing it up']);
    ...
    expect(after).toEqual(['Turno iniciado', 'Checking the repository', 'Read second.ts', 'Writing it up']);
  });
...
describe('evidence from the page context', () => {
  const GOOD = {
    ...
    index: 0,
    tool: 'agent_status',
    path: '/' + BRANDING.name + ' Core/mcp/agent_status',
    app: BRANDING.name + ' Core',
...
describe('the activity feed', () => {
  ...
  it('bounds one UTF-8 observation before it can wedge the bridge on an unhalvable 413', async () => {
    ...
    expect(delivered[0]!.event.text).toContain('Observação do navegador reduzida');
  });
...
describe('the Compact & resume control', () => {
  ...
  it('sits in the composer, before the send button once the chat exists', async () => {
    ...
    expect(control.querySelector('.clf-compact-btn')!.getAttribute('data-clf-tip')).toBe(
      'Compactação automática desativada\nObjetivo e Repetição desativados'
    );
  });
  ...
  it('opens a settings sheet with the switch, the mode slider and the compaction action', async () => {
    ...
    expect(menu.querySelector('.clf-menu-action')!.textContent).toBe('Compactar e retomar agora');
  ...
  it('locks every compaction affordance in a worker chat and emits no settings write when clicked', async () => {
    ...
    expect(auto.querySelector('.clf-menu-note')?.textContent).toMatch(/auxiliares não compactam automaticamente/i);
    auto.click();
    await settle();
    ...
    expect(action.textContent).toBe('Compactar e retomar indisponível');
    expect(action.getAttribute('data-clf-tip')).toMatch(/não podem ser compactados ou retomados manualmente/i);
    action.click();
    await settle();
  ...
  it('says an OpenRouter key is needed before the mode slider can do anything', async () => {
    ...
    expect(note.textContent).toBe('Chave do OpenRouter necessária');
  ...
  it('says what the job is doing at every stage', async () => {
    ...
    expect(state({})).toMatchObject({ mode: 'idle', label: 'Compactar', action: 'start' });
    expect(state({ disconnected: true })).toMatchObject({
      mode: 'off',
      hint: 'A conexão com o navegador está desativada no aplicativo.',
      action: 'none'
    });
    expect(state({ pressedAt: 900 })).toMatchObject({ mode: 'busy', label: 'Iniciando…', action: 'none' });
    ...
    expect(state({ job: pending, phase: 'settling' })).toMatchObject({ mode: 'busy', label: 'Concluindo…' });
    expect(state({ job: pending, phase: 'waiting' })).toMatchObject({ mode: 'busy', label: 'Escrevendo…' });
    // An unknown phase — a tab that reloaded mid-run and lost its local state — still says
    // something true rather than nothing.
    expect(state({ job: pending, phase: '' })).toMatchObject({ mode: 'busy', label: 'Solicitando…' });
    ...
    ).toMatchObject({ mode: 'waiting', label: 'Aguardando…', action: 'cancel' });
  ...
  it('says "Chat blocked" beside the gear of a chat blocked in the app, and how to release it', async () => {
    ...
    expect(label.textContent).toBe('Conversa bloqueada');
    expect(label.getAttribute('data-clf-tip')).toMatch(/Para liberar.*lista de conversas/i);
  ...
  it('starts one job on a press and refuses a second press while it runs', async () => {
    ...
    (live.document.querySelector('.clf-compact-btn') as HTMLButtonElement).click();
    expect(live.document.querySelector('.clf-menu-action')!.textContent).toBe('Cancelar compactação');
    (live.document.querySelector('.clf-menu-action') as HTMLButtonElement).click();
    await settle();
    const compacts = live.sent.filter((message) => message.type === 'compact');
    expect(startedCompactions(live)).toHaveLength(1);
...
describe('the field above the composer', () => {
  ...
  it('names the stage the transaction is in', async () => {
    ...
    expect(view({ job: { stage: 'handoff-pending', busy: true } })).toMatchObject({
      stage: 'O ChatGPT está escrevendo o resumo'
    });
    expect(view({ job: { stage: 'opening', busy: true } })).toMatchObject({ stage: 'Abrindo uma nova conversa' });
    expect(view({ job: { stage: 'waiting-for-browser', busy: true } })).toMatchObject({ stage: 'Aguardando o Chrome' });
  });
  ...
  it('stacks above the composer rather than inside it, and leaves when it is done', async () => {
    ...
    expect(live.document.querySelector('#prompt-textarea')!.textContent).toBe('');
    expect(panel.querySelector('.clf-stage-title')!.textContent).toBe('Abrindo uma nova conversa');
...
describe('folding away the chat’s opening instruction', () => {
  ...
  it('folds it away without losing a word of it', async () => {
    ...
    expect(fold.querySelector('summary')!.textContent).toContain('não foi digitado por você');
  ...
  it('says which kind of machinery it was, and which worker this chat is', async () => {
    ...
    expect(summary.querySelector('.clf-boot-label')!.textContent).toBe(
      'Este é worker-3 — instrução enviada pelo aplicativo, não por você'
    );
...
describe('the goal loop', () => {
  ...
  it('reclaims the exact retryable draft after a content-script reload', async () => {
    ...
    expect(live.document.querySelector('.clf-stage')?.textContent).toContain('Nova tentativa do objetivo em 15 segundos');
  ...
  it('keeps the turn the app still sees working, and asks again on the plain wait', async () => {
    ...
    expect(stage()).not.toContain('Nova tentativa do objetivo');
  ...
  it('shows a retryable failure as a retry, and stops saying so once the retry starts', async () => {
    ...
    expect(stage(), 'a wait nobody is told about reads as a loop that gave up').toContain('Nova tentativa do objetivo em 15 segundos');
  ...
  it('sends nothing when the model says the goal is met', async () => {
    ...
    expect(close.getAttribute('aria-label')).toBe('Fechar estado do objetivo');
    close.click();
    expect(live.document.querySelector('.clf-stage')).toBeNull();
  ...
  it('says what it is doing above the composer', async () => {
    ...
    expect(view({ phase: 'settling', error: '', model: MODEL, draft: null })).toMatchObject({
      stage: 'Verificando se a resposta terminou'
    });
    expect(view({ phase: 'requesting', error: '', model: MODEL, draft: null })).toMatchObject({
      stage: 'Enviando resposta ao OpenRouter',
    ...
    ).toMatchObject({ stage: 'deepseek-v4-flash está respondendo', body: 'what about th' });
    expect(
      view({ phase: 'drafting', error: '', model: MODEL, draft: { stage: 'ready', reply: 'what about the tests' } })
    ).toMatchObject({ stage: 'deepseek-v4-flash escreveu a próxima mensagem', body: 'what about the tests' });
    expect(
      view({ phase: 'sending', error: '', model: MODEL, draft: { stage: 'ready', reply: 'what about the tests' } })
    ).toMatchObject({ stage: 'Enviando ao ChatGPT', body: 'what about the tests' });
    expect(view({ phase: 'done', error: '', model: MODEL, draft: null })).toMatchObject({
      stage: 'Objetivo alcançado',
      detail: 'nenhuma mensagem foi enviada',
      kind: 'goal-done'
    });
    ...
    expect(view({ phase: 'failed', error: 'out_of_credit: add credits', model: MODEL, draft: null })).toMatchObject({
      stage: 'A continuidade por objetivo parou',
    ...
    expect(hook.stageView({ job: { stage: 'opening', busy: true }, goal: { phase: 'settling', model: MODEL, draft: null } })).toMatchObject({
      stage: 'Abrindo uma nova conversa'
    });
  });
  ...
  it('names the stages of a run and marks how far it got', async () => {
    ...
    expect(hook.stageView({ job: { stage: 'opening', busy: true }, goal: null, phase: 'delivering' })).toMatchObject({
      steps: ['Preparando', 'Escrevendo o resumo', 'Salvando', 'Abrindo nova conversa'],
      at: 3
    });
  });
  ...
  it('retries a rate-limited New Chat opening and sends the successful answer once', async () => {
    ...
    expect(live.document.body.textContent).toContain('Nova tentativa do objetivo em 15 segundos');
    expect(live.document.body.textContent).not.toContain('A continuidade por objetivo parou');
    wakes[0]!();
    await settle(800);
  ...
  it('opens a New Chat as a loop when the loop link wrote the goal', async () => {
    ...
    expect(loop.textContent).toContain('adicionar repetição');
    loop.click();
    ...
    expect(save.textContent).toBe('Salvar como repetição');
    save.click();
    await settle(800);
  ...
  it('follows the mode slider with an open editor, and will not save into Off', async () => {
    ...
    box.dispatchEvent(new live.window.Event('input', { bubbles: true }));
    const save = () => live!.document.querySelector('.clf-menu-goal-save') as HTMLButtonElement;
    expect(save().textContent).toBe('Salvar como objetivo');
    ...
    ).toBe('Selecione Objetivo ou Repetição primeiro. Desativado não escreve mensagens.');
  ...
  describe('the settings sheet', () => {
    ...
    it('offers one task under the slider, and shows the one a chat already has', async () => {
      ...
      expect(empty.objective.actions.map((action) => action.label)).toEqual(['adicionar tarefa']);
      ...
      expect(armed.objective.actions[0]).toMatchObject({ label: 'adicionar tarefa', mode: 'goal', disabled: false });
      ...
      expect(set.tip).toContain('Objetivo ativado');
      expect(set.mode!.value).toBe('goal');
      expect(set.mode!.note).toBe('responde até atingir o objetivo');
      expect(set.objective.actions.map((action) => action.label)).toEqual(['editar tarefa']);
    });
    ...
    it('lets a chat that answered for itself sit at Off with its task still written down', async () => {
      ...
      expect(stopped.mode!.note).toBe('sem respostas automáticas');
      expect(stopped.objective.summary).toBe('build the voxel sandbox');
      expect(stopped.objective.actions[0]).toMatchObject({ label: 'editar tarefa', disabled: true });
      expect(stopped.tip).toContain('Objetivo e Repetição desativados');
    });
    ...
    it('names the mode a chat is being driven in, and edits the task without duplicating mode controls', async () => {
      ...
      expect(sheet(carried).mode!.value).toBe('goal');
      expect(sheet(carried).objective.actions.map((action) => action.label)).toEqual(['editar tarefa']);
      ...
      expect(looping.objective.actions.map((action) => action.label)).toEqual(['editar tarefa']);
      expect(looping.objective.actions[0]!.mode).toBe('loop');
      expect(looping.mode!.note).toBe('responde até você desativar');
      expect(looping.tip).toContain('exige parada manual');
    });
    ...
    it('drops the mode slider above a New Chat and offers both modes as links instead', async () => {
      ...
      expect(fresh.tip).toContain('Adicione um objetivo ou repetição para iniciar');
    ...
    it('says why a worker chat cannot be given a goal', async () => {
      ...
      expect(view.mode).toMatchObject({ value: 'off', note: 'o principal conduz esta conversa', warn: true, disabled: true });
      expect(view.tip).toContain('o principal conduz esta conversa');
      expect(view.objective).toMatchObject({
        available: false,
        unavailable: 'A conversa auxiliar já é conduzida pelo principal.'
      });
    });
    ...
    it('takes every control away from a chat the user blocked in the app, and says so', async () => {
      ...
      expect(view.tip).toContain('bloqueada no aplicativo');
      expect(view.rows[0]).toMatchObject({
        key: 'autoCompact',
        on: false,
        disabled: true,
        note: expect.stringMatching(/bloqueada no aplicativo/)
      });
      expect(view.mode).toMatchObject({ value: 'off', note: 'bloqueada no aplicativo', warn: true, disabled: true });
      expect(view.objective).toMatchObject({ available: false });
      expect(view.objective.unavailable).toMatch(/bloqueada no aplicativo/);
      expect(view.action).toMatchObject({ label: 'Compactar e retomar indisponível', action: 'none' });
    });
    ...
    it('keeps pointing at the missing credential the whole feature runs on', async () => {
      ...
      expect(view.mode).toMatchObject({ note: 'Chave do OpenRouter necessária', warn: true });
      expect(view.objective).toMatchObject({
        available: false,
        unavailable: 'Adicione uma chave de API do OpenRouter no aplicativo primeiro.'
      });
    });

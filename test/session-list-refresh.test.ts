...
describe('visible Chat refresh', () => {
  ...
  it('loads an older session page on scroll and replaces the misleading visible-count footer', async () => {
    ...
    await vi.waitFor(() =>
      expect(w.document.getElementById('sessionsFoot')?.textContent).toContain('60 exibidas de 65 conversas mantidas')
    );
    ...
    expect(w.document.getElementById('sessionsFoot')?.textContent).toContain('65 conversas mantidas');
  });
});

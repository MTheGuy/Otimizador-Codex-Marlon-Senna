...
it('preserves the draft and refuses send while model discovery has no confirmed choices', async () => {
  ...
  expect(w.document.body.textContent).toContain('Recarregar modelos');
...
it('folds a whole Compact & Resume into one row that says the new chat opened', async () => {
  ...
  expect(card.querySelector('summary')!.textContent).toMatch(/^Compactar e continuar:Nova conversa aberta às .* \(44 caracteres\)$/);
  ...
  expect(card.textContent).toContain('Solicitação de resumo');
  expect(card.textContent).toContain('keep the loop running');
  expect(card.textContent).toContain('Resumo salvo');
  expect(card.textContent).toContain('Contexto inicial enviado à nova conversa');
});
...
it.each(['composer', 'bubble'])('clears New Chat drafts and removes a delivery cancelled with %s', async (control) => {
  ...
  expect(w.document.getElementById('chatSend')!.getAttribute('aria-label')).toBe('Cancelar envio');
  if (control === 'composer') w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  else (w.document.querySelector('#inputQueue [title="Cancelar envio"]') as HTMLButtonElement).click();
  await settle();
...
it.each([false, true])('dismisses retired delivery errors in selected-chat=%s without resending', async selected => {
  ...
  const retry = queue.querySelector<HTMLButtonElement>('[aria-label="Tentar enviar novamente"]')!;
  ...
  expect(w.document.getElementById('chatSend')!.getAttribute('aria-label')).not.toBe('Cancelar envio');
  (queue.querySelector('[title="Ocultar aviso de entrega"]') as HTMLButtonElement).click();
  await app.append([]);
...
it('shows a compaction that never made it into a new chat as failed once the chat moved on', async () => {
  ...
  expect(state()).toBe('Resumo escrito — salvando a continuidade…');
  ...
  expect(state()).toBe('Resumo escrito, mas não salvo pelo aplicativo — a conversa continuou aqui');
...
it('says why a compaction died when the app abandoned it', async () => {
  ...
  expect(card.querySelector('summary .state')!.textContent).toBe('Falhou — the handover never landed and was given up on');
...
it('keeps an unfolded tool row as the same open node while the chat keeps appending', async () => {
  ...
  expect(group.querySelector('summary')!.title).toContain('4 ações');
});
...
it('keeps mixed tool and agent activity in one latest-action disclosure between authored messages', async () => {
  ...
  expect(group.querySelector('.agent-communication summary')!.textContent).toContain('Mensagem de worker-2');
...
it('offers deliberate helper retry only for the selected source session', async () => {
  ...
  const queue = w.document.getElementById('inputQueue')!;
  expect(queue.textContent).toContain('A conversa anterior ainda pode estar em execução.');
...
it('updates an offered injection receipt in its existing transcript position', async () => {
  ...
  expect(app.w.document.querySelector('.input-receipt')?.getAttribute('aria-label')).toBe('Enviado ao turno ativo · aguardando confirmação');
  ...
  expect(app.w.document.querySelector('.input-receipt')?.getAttribute('aria-label')).toBe('Entrega confirmada');
});
...
it.each(['menu', 'send'])('queues an Astra finish message through %s using the existing staged queue', async action => {
  ...
  expect(w.document.getElementById('afterTurnLabel')!.textContent).toBe('Colocar na fila ao encerrar a sessão');
  const input = w.document.getElementById('chatInput') as HTMLTextAreaElement;
...
it('shows elapsed work for the exact recorded turn without exposing lifecycle rows', async () => {
  ...
  expect(w.document.getElementById('chatState')!.textContent).toMatch(/^Trabalhando por /);
  (w as any).api.getSessionControls = (id: string) => Promise.resolve({ ok: true, data: { sessionId: id, automation: 'off', activeTurnId: null, finishHeld: false, blocked: '', job: null } });
  await append([{ seq: 2, time: T0 + 65_000, source: 'extension', kind: 'turn_end', turnId: 'held-turn', outcome: 'completed' }]);
  expect(w.document.getElementById('chatState')!.textContent).toBe('Trabalhou por 1m 5s');
});
...
it('stops directly from the empty composer without a second Stop menu action', async () => {
  ...
  expect(send.getAttribute('aria-label')).toBe('Parar turno');
  ...
  expect(send.getAttribute('aria-label')).toBe('Enviar mensagem');
...
it('shows Stop immediately for a queued first send, switches to Send for a new draft, and cancels through the same button', async () => {
  ...
  await settle();
  expect(send.dataset.action).toBe('stop');
  expect(w.document.querySelector('#inputQueue button')?.textContent).not.toBe('Cancelar');
...
it('keeps editable stages until the composer sends the first stage exactly once', async () => {
  ...
  await settle();
  expect(live.sent).toHaveLength(0);
  expect(w.document.getElementById('taskPlanPreview')!.textContent).not.toMatch(/Iniciar plano|Revisar etapas/);
  const edit = w.document.querySelector<HTMLButtonElement>('[aria-label="Editar etapa 1"]')!;
  edit.click();
  const stage = w.document.querySelector<HTMLTextAreaElement>('textarea[aria-label="Editar etapa 1"]')!;
  stage.value = 'Build the edited foundation'; stage.dispatchEvent(new w.Event('input'));
  expect(w.document.getElementById('chatSend')!.getAttribute('aria-label')).toBe('Enviar primeira etapa');
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  w.document.getElementById('composer')!.dispatchEvent(new w.Event('submit', { cancelable: true }));
  await settle();
...
it('disables empty task actions and confirms saving without the old helper sentence', async () => {
  ...
  expect((w.document.getElementById('chatInput') as HTMLTextAreaElement).placeholder).toContain('plano');
  expect(plan.getAttribute('aria-pressed')).toBe('true');
  plan.click();
  expect(plan.getAttribute('aria-pressed')).toBe('false');
  expect((w.document.getElementById('chatInput') as HTMLTextAreaElement).placeholder).toBe('Escreva sua mensagem…');
  const objective = w.document.getElementById('sessionObjective') as HTMLTextAreaElement;
  ...
  expect(save.textContent).toContain('Salvo'); expect(save.disabled).toBe(true);
  ...
  expect(save.disabled).toBe(false); expect(save.textContent).not.toContain('Salvo');
});
...
it('dismisses only the displayed recovery revision and reveals a changed status or new incident', async () => {
  ...
  const dismiss = () => (status.querySelector('[aria-label="Ocultar aviso de recuperação"]') as HTMLButtonElement).click();
...
it('blocks an empty stage, deletes it explicitly, and hides the whole dock in settings', async () => {
  ...
  w.document.querySelector<HTMLButtonElement>('[aria-label="Excluir etapa 1"]')!.click();
...
it('clearing the complete planner task cancels generation and restores Create plan', async () => {
  ...
  expect(w.document.getElementById('createPlan')!.textContent).toBe('Criar plano');
...
it('routes an armed empty-composer plan through the planner and paints only its live progress', async () => {
  ...
  expect(w.document.getElementById('taskPlanPreview')!.textContent).toContain('Provedor ocupado · nova tentativa 2');
...
it('streams a new Goal opening, queues it once, and displays authoritative delivery failure', async () => {
  ...
  expect(row.textContent).toContain('Preparando a mensagem inicial');
  ...
  expect(row.textContent).toContain('Provedor ocupado · nova tentativa 2');
  expect(row.getAttribute('aria-busy')).toBe('true');
  ...
  expect(row.textContent).toContain('Mensagem inicial na fila');
...
it('renders existing-chat Goal draft stages from main controls without starting another request', async () => {
  ...
  expect(row.textContent).toContain('aguardando envio ao ChatGPT');
...
it('shows the active finish animation after an earlier ordinary Goal save failed', async () => {
  ...
  w.document.getElementById('saveSessionObjective')!.click(); await settle();
  const row = w.document.getElementById('goalLifecycle')!;
  expect(row.textContent).toContain('Não foi possível continuar a tarefa');
  ...
  expect(row.textContent).not.toContain('Não foi possível continuar a tarefa');
  expect(row.getAttribute('aria-busy')).toBe('true');

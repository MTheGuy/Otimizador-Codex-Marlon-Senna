...
let currentWindow: {
  ...
  isDestroyed: () => boolean;
  webContents: { send: ReturnType<typeof vi.fn>; mainFrame: object };
} | null = null;

function createWindowFixture() {
  return {
    setBackgroundColor: vi.fn(), isDestroyed: () => false,
    webContents: { send: vi.fn(), mainFrame: {} }
  };
}

function rendererEvent() {
  if (!currentWindow) throw new Error('A fixture precisa de uma janela principal ativa.');
  return { sender: currentWindow.webContents, senderFrame: currentWindow.webContents.mainFrame };
}

const save = (patch: unknown, base: unknown = getConfig()): Promise<any> =>
  handlers.get('settings:save')!(rendererEvent(), { patch, base }) as Promise<any>;
const renameRoot = (payload: unknown): Promise<any> => handlers.get('roots:rename')!(rendererEvent(), payload) as Promise<any>;
const removeRoot = (payload: unknown): Promise<any> => handlers.get('roots:remove')!(rendererEvent(), payload) as Promise<any>;
const sessionEvents = (payload: unknown): Promise<any> => handlers.get('sessions:events')!(rendererEvent(), payload) as Promise<any>;
const sessionList = (): Promise<any> => handlers.get('sessions:list')!(rendererEvent(), undefined) as Promise<any>;

it('validates dropped image count and decodes bytes through the existing image authority', async () => {
  const drop = (payload: unknown) => handlers.get('sessions:dropImages')!(rendererEvent(), payload) as Promise<any>;
...
it('does not authorize the composer Generate Goal action from an absent or stale finish wait', async () => {
  const generate = (payload: unknown) => handlers.get('sessions:generateFinishGoal')!(rendererEvent(), payload) as Promise<any>;
...
it('native opening cancellation aborts the exact IPC invocation and prevents a late ready result', async () => {
  ...
  try {
    const opening = handlers.get('sessions:goalOpening')!(rendererEvent(), { text: 'Implement safely', mode: 'goal', requestId });
    const cancelled = await handlers.get('tasks:cancel')!(rendererEvent(), { requestId }) as any;
...
it('adds picker-selected projects, reuses containing approval, and leaves cancellation unchanged', async () => {
  currentWindow = createWindowFixture();
  ...
  const add = () => handlers.get('projects:add')!(rendererEvent(), {}) as Promise<any>;
  ...
  expect(getConfig().roots).toHaveLength(1);
  const listed = await handlers.get('projects:list')!(rendererEvent(), {}) as any;
...
describe('autoridade da janela que solicita IPC', () => {
  it('aceita o frame principal da janela ativa', async () => {
    const reply = await handlers.get('swarm:get')!(rendererEvent(), undefined);
    expect(reply).toMatchObject({ ok: true });
  });

  it.each(['evento ausente', 'outra janela', 'subframe', 'janela destruída', 'sem janela'])(
    'recusa %s antes de abrir qualquer endereço', async scenario => {
      const event = rendererEvent();
      let incoming: unknown = event;
      if (scenario === 'evento ausente') incoming = null;
      if (scenario === 'outra janela') {
        const other = createWindowFixture();
        incoming = { sender: other.webContents, senderFrame: other.webContents.mainFrame };
      }
      if (scenario === 'subframe') incoming = { sender: event.sender, senderFrame: {} };
      if (scenario === 'janela destruída') currentWindow!.isDestroyed = () => true;
      if (scenario === 'sem janela') currentWindow = null;
      const reply = await handlers.get('link:open')!(incoming, { url: 'https://openrouter.ai/settings/keys' });
      expect(reply).toMatchObject({ ok: false, error: expect.stringContaining('origem não autorizada') });
...
describe('explicit settings replace the published tool contract', () => {
  it.each(['finish', 'command', 'session'] as const)('withdraws %s from real endpoint publication after its setting is disabled', async kind => {
    ...
    await saveConfig({ ...initial, readOnly: false, ui: { ...initial.ui, finishTool: true }, capabilities: { ...initial.capabilities, read: true, command: true } });
...
describe('startup state without secure storage', () => {
  it('still returns a usable app/bridge state instead of crashing state discovery', async () => {
    ...
    const reply = (await handlers.get('state:get')!(rendererEvent(), undefined)) as any;
...
describe('turning multi-agent mode off', () => {
  ...
  it('keeps disabled history until the explicit Clear swarm IPC destroys it', async () => {
    ...
    const cleared = await handlers.get('swarm:reset')!(rendererEvent(), undefined) as any;
...
describe('bounded IPC identities and OS launch results', () => {
  it('reports shell.openPath failure instead of claiming the extension folder opened', async () => {
    ...
    const reply = (await handlers.get('bridge:openExtensionFolder')!(rendererEvent(), undefined)) as {
      ok: boolean;
      error?: string;
    };
  ...
  it('opens the extension recovery ZIP from the installed app version, never releases/latest', async () => {
    ...
    const version = await import('../src/main/version.js');
    const url = 'https://example.invalid/releases/download/v1.8.8/extension.zip';
    const configured = vi.spyOn(version, 'extensionDownloadUrl').mockReturnValue(url);
    try {
      const reply = await handlers.get('bridge:downloadExtension')!(rendererEvent(), undefined);
      expect(reply).toEqual({ ok: true, data: true });
      expect(configured).toHaveBeenCalledWith('1.8.8');
      expect(shell.openExternal).toHaveBeenCalledWith(url);
      expect(vi.mocked(shell.openExternal).mock.calls[0]?.[0]).not.toContain('/releases/latest/');
    } finally { configured.mockRestore(); }
  });

  it('não abre uma distribuição externa quando nenhum endereço foi configurado', async () => {
    const version = await import('../src/main/version.js');
    const unavailable = vi.spyOn(version, 'extensionDownloadUrl').mockReturnValue('');
    try {
      const reply = await handlers.get('bridge:downloadExtension')!(rendererEvent(), undefined);
      expect(reply).toMatchObject({ ok: false, error: expect.stringContaining('Nenhum endereço') });
      expect(shell.openExternal).not.toHaveBeenCalled();
  ...
  it('bounds and validates an agent id before it reaches the global broker', async () => {
    ...
    const oversized = (await clear(rendererEvent(), 'worker-' + 'x'.repeat(200_000))) as { ok: boolean; error?: string };
    expect(oversized.ok).toBe(false);
    expect(oversized.error).toMatch(/64|too big/i);

    const punctuation = (await clear(rendererEvent(), 'worker-1\nspoofed')) as { ok: boolean; error?: string };
...
describe('settings writes from more than one UI', () => {
  ...
  it('não restaura chamadas sem autoria após salvar uma preferência visual desatualizada', async () => {
    const base = defaultConfig();
    await saveConfig(base);
    await saveConfig({
      ...base,
    ...
    expect(getConfig().multiAgent.allowUnattributedCalls).toBe(false);
  });
...
describe('root namespace invariants', () => {
  it('approves a dropped folder path exactly like the picker, and refuses a dropped file', async () => {
    ...
    const addPath = (payload: unknown): Promise<any> => handlers.get('roots:addPath')!(rendererEvent(), payload) as Promise<any>;
  ...
  it('refuses a live rename into the reserved /skills namespace', async () => {
    ...
    expect(reply.ok).toBe(false);
    expect(reply.error).toMatch(/\/skills é reservado pelo aplicativo/);
...
describe('every link the window offers', () => {
  ...
  it('opens the OpenRouter key page the goal loop sends people to', async () => {
    ...
    expect(await open(rendererEvent(), { url: 'https://openrouter.ai/settings/keys' })).toEqual({ ok: true, data: true });
    const refused = (await open(rendererEvent(), { url: 'https://example.com/' })) as { ok: boolean; error: string };
    expect(refused.ok).toBe(false);
  ...
  it('serializes non-Error throws into a real IPC error string', async () => {
    ...
    const reply = (await handlers.get('link:open')!(rendererEvent(), {
...
it('não publica canais ou estado de atualização automática', async () => {
  expect([...handlers.keys()].filter(channel => channel.startsWith('update:'))).toEqual([]);
  const reply = await handlers.get('state:get')!(rendererEvent(), undefined) as any;
  expect(reply.ok).toBe(true);
  expect(reply.data).not.toHaveProperty('update');
});
...
describe('session IPC contracts', () => {
  ...
  it('blocks and releases the stored conversation, and never a renderer-supplied one', async () => {
    ...
    const blocked = (await handlers.get('sessions:block')!(rendererEvent(), { id: session.id, blocked: true })) as any;
    ...
    const released = (await handlers.get('sessions:block')!(rendererEvent(), { id: session.id, blocked: false })) as any;
    ...
    const refused = (await handlers.get('sessions:block')!(rendererEvent(), { id: unattributed.id, blocked: true })) as any;
    expect(refused.ok).toBe(false);
  ...
  it('releases a block when the row that carries its button is deleted', async () => {
    ...
    await handlers.get('sessions:block')!(rendererEvent(), { id: session.id, blocked: true });
    expect(isChatBlocked(conversationId)).toBe(true);

    const deleted = (await handlers.get('sessions:delete')!(rendererEvent(), { id: session.id })) as any;
  ...
  it('opens only the stored conversation URL in Chrome', async () => {
    ...
    const reply = await handlers.get('sessions:openChat')!(rendererEvent(), { id: session.id }) as any;
    expect(reply.ok, reply.error).toBe(true);
    ...
    const unattributed = await createSession({ title: 'no conversation', conversationId: null });
    const refused = await handlers.get('sessions:openChat')!(rendererEvent(), { id: unattributed.id }) as any;
    expect(refused.ok).toBe(false);
    expect(refused.error).toMatch(/no valid ChatGPT conversation/i);
  });
});
...
describe('renderer pushes after the window is gone', () => {
  it('does not touch a destroyed BrowserWindow, whose members all throw', async () => {
    ...
    } as unknown as NonNullable<typeof currentWindow>;

    currentWindow = destroyed;
...
describe('Stop IPC exact session and turn authority', () => {
  it('requires an explicit current turn and cannot stop a replacement conversation', async () => {
    const invoke = (payload: unknown) => handlers.get('sessions:stopTurn')!(rendererEvent(), payload) as Promise<any>;

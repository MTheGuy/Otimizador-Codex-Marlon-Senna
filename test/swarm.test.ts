...
describe('an ordinary call from a worker', () => {
  ...
  it('refuses a control call it cannot place, by name', () => {
    ...
    expect(refusal).toContain('Nenhuma operação de agente foi realizada');
    expect(refusal).toContain('a entrega não foi confirmada');
    expect(refusal).not.toMatch(/recarrega.*conversa|trabalho.*perdido/i);
  });

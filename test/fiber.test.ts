...
beforeAll(async () => {
  source = (await Promise.all(['branding.js', 'fiber.js'].map(file => fs.readFile(path.join(process.cwd(), 'extension', file), 'utf8')))).join('\n');
});
 ...
 */
const APP = BRANDING.name + ' Core';
const DESKTOP_APP = BRANDING.name + ' Desktop';
/** What the connector was called before 1.7.1 split it. Older chats still hold it. */
const LEGACY_APP = 'Outro aplicativo';
...
describe('the calls a turn says it made', () => {
   ...
   */
  it('reconhece os dois conectores configurados e recusa outra integração', async () => {
    const messages = [
  ...
  it('refuses a connector whose name merely starts with this app’s brand', async () => {
    const messages = [
      request('req-fake', 'read', { app: BRANDING.name + ' Backup' }),
      answer('res-fake', 'req-fake', 'read', BRANDING.name + ' Backup'),

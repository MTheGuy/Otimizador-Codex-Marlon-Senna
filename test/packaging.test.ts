...
describe('cross-platform packaging targets', () => {
  ...
  it('grants sandbox read access only to the Windows install tree and fails on ACL errors', () => {
    ...
    expect(installer).toContain('Abort "O Windows não conseguiu configurar o acesso à pasta necessário');
  ...
  it('assembles every platform artifact in the reusable release workflow', () => {
    ...
    const expression = parsed.jobs.package.strategy.matrix.include as string;
    const variants = expression.match(/fromJSON\(inputs\.all_platforms && '([^']+)' \|\| '([^']+)'\)/)!;
    expect(variants).not.toBeNull();
    const matrix = JSON.parse(variants[1]!);
    expect(JSON.parse(variants[2]!)).toEqual([matrix[0]]);
    expect(parsed.on.workflow_dispatch.inputs.all_platforms.default).toBe(false);
    expect(parsed.on.workflow_call.inputs.all_platforms.default).toBe(false);
    expect(matrix).toHaveLength(6);
    expect(matrix).toEqual([
      {
        name: 'Windows x64', platform: 'win32', arch: 'x64', runner: 'windows-2025',
        script: 'dist:x64', artifact: 'package-windows-x64', files: 'release/*-Setup-x64.exe'
      },
      {
        name: 'Windows arm64', platform: 'win32', arch: 'arm64', runner: 'windows-11-arm',
        script: 'dist:arm64', artifact: 'package-windows-arm64', files: 'release/*-Setup-arm64.exe'
      },
      {
        name: 'macOS x64', platform: 'darwin', arch: 'x64', runner: 'macos-15-intel',
        script: 'dist:mac:x64', artifact: 'package-macos-x64',
        files: 'release/*-macOS-x64.dmg\nrelease/*-macOS-x64.zip\n'
      },
      {
        name: 'macOS arm64', platform: 'darwin', arch: 'arm64', runner: 'macos-15',
        script: 'dist:mac:arm64', artifact: 'package-macos-arm64',
        files: 'release/*-macOS-arm64.dmg\nrelease/*-macOS-arm64.zip\n'
      },
      {
        name: 'Linux x64', platform: 'linux', arch: 'x64', runner: 'ubuntu-24.04',
        script: 'dist:linux:x64', artifact: 'package-linux-x64',
        files: 'release/*-Linux-x64.AppImage\nrelease/*-Linux-x64.deb\n'
      },
      {
        name: 'Linux arm64', platform: 'linux', arch: 'arm64', runner: 'ubuntu-24.04-arm',
        script: 'dist:linux:arm64', artifact: 'package-linux-arm64',
        files: 'release/*-Linux-arm64.AppImage\nrelease/*-Linux-arm64.deb\n'
      }
    ]);
    expect(parsed.jobs.package['runs-on']).toBe('${{ matrix.runner }}');
    expect(workflow).toContain('name: candidato-${{ github.run_id }}');
    ...
    expect(workflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a "/usr/bin/$APP_PACKAGE_NAME"');
    ...
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$mount_dir/${APP_PRODUCT_NAME}.app\"");
    expect(workflow).toContain("node scripts/smoke-macos-bundle.mjs '${{ matrix.arch }}' \"$zip_dir/${APP_PRODUCT_NAME}.app\"");
    ...
    expect(workflow).toContain('grep -Fxq "Name=$APP_PRODUCT_NAME" "$desktop"');
    expect(workflow).toContain('grep -Fxq "Icon=$APP_PACKAGE_NAME" "$desktop"');
    ...
    expect(debGui).toContain('xvfb-run -a "/usr/bin/$APP_PACKAGE_NAME"');
  ...
  it('uses Noble-compatible Linux packages and a FUSE-independent AppImage runtime', () => {
    ...
    expect(builder.linux.artifactName).toBe(BRANDING.installerPrefix + '-Linux-${env.COS_PACKAGE_ARCH}.${ext}');
    ...
    expect(builder.linux.maintainer).toBe(BRANDING.author);
    expect(pkg.desktopName).toBe(BRANDING.appId + '.desktop');
    expect(pkg.homepage ?? '').toBe(BRANDING.homepageUrl);
    expect(iconScript).toContain("['build/icon.png', await pngAt(appImage, 1024)]");
    ...
    expect(releaseWorkflow).toContain('CLF_DEBUG=1 timeout --signal=TERM --kill-after=5s 12s xvfb-run -a "/usr/bin/$APP_PACKAGE_NAME"');
    ...
    expect(releaseWorkflow).toContain('test "$(dpkg-deb --field "$deb" Package)" = "$APP_PACKAGE_NAME"');
    ...
    expect(releaseWorkflow).toContain('test -L "/usr/bin/$APP_PACKAGE_NAME"');
    expect(releaseWorkflow).toContain('installed_executable="$(readlink -f "/usr/bin/$APP_PACKAGE_NAME")"');
    ...
    expect(notes).toMatch(/não foram produzidos nem validados/i);
  });
  ...
  it('pins the current macOS release to unsigned thin native bundles with explicit metadata checks', () => {
    ...
    expect(builder.mac.artifactName).toBe(BRANDING.installerPrefix + '-macOS-${arch}.${ext}');
    ...
    for (const marker of [
      'CFBundleIdentifier: branding.appId',
      'CFBundleExecutable: branding.executableName',
      'CFBundleName: branding.name',
      'CFBundleDisplayName: branding.name',
  ...
  it('hides Electron helper parentheses from otool-classic without changing the inspected file', () => {
    const file = '/Applications/${APP_PRODUCT_NAME}.app/Contents/Frameworks/Chat On Steroids Helper (GPU).app/Contents/MacOS/Chat On Steroids Helper (GPU)';
...
`;
  ...
  it('rejects invalid publishes before allocating the reusable six-runner build', () => {
    ...
    expect(workflow.slice(preflight, candidate)).toContain('APP_VERSION = BRANDING.version;');
    expect(workflow.slice(preflight, candidate)).toContain('branding.publication.repository !== process.env.GITHUB_REPOSITORY');
    expect(workflow.slice(preflight, candidate)).toContain('artefatos macOS não têm assinatura de publicação nem notarização');
  ...
  it('keeps the current changelog and reviewed release notes aligned with every published artifact', () => {
    ...
    expect(versionSource).toContain('APP_VERSION = BRANDING.version;');
    expect(APP_VERSION).toBe(BRANDING.version);
    expect(APP_VERSION).toBe(pkg.version);
    expect(changelog.match(/^## \[?(\d+\.\d+\.\d+)\]?/m)?.[1]).toBe(pkg.version);
    expect(notes).toContain(`## ${pkg.version}`);
    const artifacts = [
      BRANDING.installerPrefix + '-Setup-x64.exe',
      BRANDING.installerPrefix + '-Extension.zip',
      'SHA256SUMS.txt'
    ];
    for (const artifact of artifacts) expect(notes).toContain(`\`${artifact}\``);
    ...
    expect(checksumStep).toContain('"${APP_INSTALLER_PREFIX}-Setup-x64.exe"');
    expect(checksumStep).toContain('"${APP_INSTALLER_PREFIX}-Extension.zip"');
    expect(checksumStep).toContain('if [ "$ALL_PLATFORMS" = true ]; then');
    for (const suffix of ['Setup-arm64.exe', 'macOS-x64.dmg', 'macOS-x64.zip', 'macOS-arm64.dmg',
      'macOS-arm64.zip', 'Linux-x64.AppImage', 'Linux-x64.deb', 'Linux-arm64.AppImage', 'Linux-arm64.deb']) {
      expect(checksumStep).toContain('${APP_INSTALLER_PREFIX}-' + suffix);
    }
    expect(checksumStep).toContain('test -f "release/$file"');
    expect(checksumStep).toContain('sha256sum "${files[@]}" > SHA256SUMS.txt');
    const candidateUpload = release.slice(release.indexOf('      - name: Upload release candidate'));
    expect(candidateUpload).toContain('release/${{ steps.identity.outputs.prefix }}-*');
    ...
    expect(publishStep).toContain('done < publish/SHA256SUMS.txt');
    expect(publishStep).toContain('gh release create "$TAG" "${files[@]}" publish/SHA256SUMS.txt');

...
describe('spawning a run', () => {
  it('refuses the feature while it is switched off', async () => {
    ...
    expect(() => spawn({ workers: [{ task: 'x' }], caller: prime })).toThrow(/modo multiagente está desativado/i);
    await setEnabled(true);
  });
...
describe('through the MCP endpoint', () => {
  ...
  it('mantém chamadas sem atribuição bloqueadas e preserva o bloqueio de um executor aposentado', async () => {
    startSwarm(1);
    ...
    expect(anonymous).toContain('CALLER_IDENTITY_REQUIRED');
    expect(anonymous).not.toMatch(REFUSED_ON_ROOTS);
  ...
  it('recusa uma chamada dependente de projeto sem atribuição em vez de adivinhar a conversa', async () => {
    await setEnabled(true, 3, true);
    startSwarm(1);
  ...
  it('refuses a control call it cannot place at all, and says where to look', async () => {
    ...
    expect(text).toMatch(/Nenhuma operação de agente foi realizada/);
    expect(text).not.toContain('worker-1');
  });

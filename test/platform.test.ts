...
import { DEFAULT_CAPABILITIES, type Capabilities } from '../src/shared/types.js';
...
describe('cross-platform product surface', () => {
  it('starts macOS in STRICT mode and preserves explicit Desktop grants', () => {
    ...
    const switchedOn = { ...config, capabilities: allCapabilities(), readOnly: false };
  ...
  it('starts Linux in STRICT mode while allowing Core through explicit grants', () => {
    ...
    const permitted = { ...config, capabilities: allCapabilities(), readOnly: false };
    expect(effectiveCapabilities(permitted, 'linux').command).toBe(true);
  });
  ...
  it.each(['darwin', 'linux'] as const)('teaches POSIX shell semantics instead of Windows guidance on %s', (platform) => {
    ...
    expect(instructions.includes(`${BRANDING.name} Desktop`)).toBe(platform === 'darwin');
  });
  ...
  it('retains the Windows-specific shell guidance on Windows', () => {
    ...
    expect(instructions).toContain(`${BRANDING.name} Desktop`);
  });

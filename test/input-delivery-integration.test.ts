...
let bearer: string;
const pushed = vi.fn();
const currentWindow = { isDestroyed: () => false, webContents: { send: pushed, mainFrame: {} } };
const rendererEvent = () => ({ sender: currentWindow.webContents, senderFrame: currentWindow.webContents.mainFrame });
...
beforeAll(async () => {
  ...
  registerIpc(() => currentWindow as never);
  await startBridge();
...
it('revokes a claimed send via IPC, fences pre-send authorization and records a late exact receipt', async () => {
  ...
  expect((await handlers.get('sessions:cancelInput')!(rendererEvent(), { id: row.id })).ok).toBe(true);
...
describe('IPC input delivery and Goal control integration', () => {
  ...
  it.each(['browser', 'tool'] as const)('records %s receipt text and pixels through the real IPC hook', async (transport) => {
    ...
    expect((await handlers.get('sessions:send')!(rendererEvent(), authored)).ok).toBe(true);
    ...
    expect((await handlers.get('sessions:image')!(rendererEvent(), { id: session.id, assetId })).data).toBe(dataUrl);
  ...
  it('publishes only nonce-bound catalog observations through HTTP and pushes completion', async () => {
    ...
    expect((await handlers.get('chatModels:request')!(rendererEvent(), {})).data.state).toBe('pending');
    ...
    expect((await handlers.get('chatModels:get')!(rendererEvent(), {})).data.models).toEqual(models);
  ...
  it('releases only the expected actual turn through IPC', async () => {
    ...
    const current = await handlers.get('sessions:controls')!(rendererEvent(), { id: session.id });
    expect(current.data).toMatchObject({ activeTurnId: 'first-held-turn', finishHeld: true });
    expect((await handlers.get('sessions:releaseFinish')!(rendererEvent(), { id: session.id, expectedTurnId: 'stale-turn' })).ok).toBe(false);
    const released = await handlers.get('sessions:releaseFinish')!(rendererEvent(), { id: session.id, expectedTurnId: 'first-held-turn' });
    ...
    expect((await handlers.get('sessions:releaseFinish')!(rendererEvent(), { id: session.id, expectedTurnId: 'first-held-turn' })).ok).toBe(false);
    expect((await handlers.get('sessions:controls')!(rendererEvent(), { id: session.id })).data.finishHeld).toBe(true);
  });
  it('shares selected-chat objectives with the extension and projects objective-only Goal', async () => {
    ...
    const call = (name: string, extra = {}) => handlers.get(name)!(rendererEvent(), { id: session.id, ...extra });
  ...
  it('controls exact durable sessions and withdraws Goal without sending new input', async () => {
    ...
    for (const channel of ['sessions:controls', 'sessions:automation', 'sessions:compact', 'sessions:cancelCompaction']) {
      expect((await handlers.get(channel)!(rendererEvent(), { id: randomUUID(), automation: 'goal' })).ok).toBe(false);
    }
  });
  it('rejects a superseded current attachment before changing either control ledger', async () => {
    ...
    try {
      for (const channel of ['sessions:controls', 'sessions:automation', 'sessions:compact', 'sessions:cancelCompaction']) {
        expect((await handlers.get(channel)!(rendererEvent(), { id: session.id, automation: 'goal' })).error).toBe('conversation_superseded');
      }
  ...
  it('uses one idempotent continuation ticket and permits cancellation while blocked', async () => {
    ...
    const first = await handlers.get('sessions:compact')!(rendererEvent(), { id: session.id });
    expect(first.ok).toBe(true);
    expect(first.data.job.token).toBeTruthy();
    const repeated = await handlers.get('sessions:compact')!(rendererEvent(), { id: session.id });
    expect(repeated.data.job.token).toBe(first.data.job.token);
    setChatBlocked(conversationId, true);
    expect((await handlers.get('sessions:compact')!(rendererEvent(), { id: session.id })).error).toBe('chat_blocked');
    expect((await handlers.get('sessions:automation')!(rendererEvent(), { id: session.id, automation: 'goal' })).error).toBe('chat_blocked');
    expect((await handlers.get('sessions:automation')!(rendererEvent(), { id: session.id, automation: 'off' })).ok).toBe(true);
    expect((await handlers.get('sessions:cancelCompaction')!(rendererEvent(), { id: session.id })).ok).toBe(true);
  ...
  it('fences durable decision helpers from Goal and compaction after reload', async () => {
    ...
    expect((await handlers.get('sessions:controls')!(rendererEvent(), { id: session.id })).data.blocked).toBe('worker');
    expect((await handlers.get('sessions:automation')!(rendererEvent(), { id: session.id, automation: 'loop' })).error).toBe('worker_goal_disabled');
    expect((await handlers.get('sessions:compact')!(rendererEvent(), { id: session.id })).error).toBe('worker_compaction_disabled');
    expect((await handlers.get('sessions:automation')!(rendererEvent(), { id: session.id, automation: 'off' })).ok).toBe(true);
  });
  ...
  it('enables automation only when an existing chat receives input, retiring its old final', async () => {
    ...
    const enqueued = await handlers.get('sessions:send')!(rendererEvent(), request);
  ...
  it('binds a new chat through HTTP ACK, applies its choice once, and pushes session change', async () => {
    const request = message(null, 'goal');
    await handlers.get('sessions:send')!(rendererEvent(), request);

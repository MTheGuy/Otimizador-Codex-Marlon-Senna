...
it('edits the canonical formula controls and per-model rates without reloading recordings, then restores preferences', async () => {
  ...
  for (const label of ['Pesquisa aprofundada', 'Envio de arquivos', 'Arquivos de texto colados', 'Geração de imagens']) expect(balances.textContent).toContain(label);
  ...
  expect(cost()).toContain(amount(0.48)); expect(cost()).toContain('sem tarifa');
  change(divisor, '4');
  expect(cost()).toContain(amount(0.24));
  ...
  expect(cost()).toContain(amount(0.24));
  const unknownRate = dom.window.document.querySelector('input[aria-label="another-model — USD por milhão de tokens de entrada em cache"]') as HTMLInputElement;
  change(unknownRate, '1');
  expect(cost()).toContain(amount(0.84)); expect(cost()).not.toContain('sem tarifa');
  change(field('usageMultiplier'), '1');
  expect(cost()).toContain(amount(0.70));

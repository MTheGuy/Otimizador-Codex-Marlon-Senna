...
async function hello(candidate) {
  try {
    ...
    return body && body.app === APP_BRANDING.appId ? body : null;
  } catch {
    return null;
  }
}
...
async function restoreChatgptTab(id) {
  try {
    ...
    if (live && live.ok === true && live.recorderVersion === PAGE_RECORDER_VERSION) {
      ...
      try {
        await chrome.scripting.executeScript({ target: { tabId: id }, world: 'MAIN', files: ['branding.js', 'fiber.js'] });
      } catch {
  ...
  try {
    ...
    await chrome.scripting.executeScript({ target: { tabId: id }, files: ['branding.js', 'chatgpt-dom.js'] });
    ...
    await chrome.scripting.executeScript({ target: { tabId: id }, world: 'MAIN', files: ['branding.js', 'fiber.js'] });
    await chrome.scripting.executeScript({ target: { tabId: id }, files: ['branding.js', 'content.js'] });

...
(() => {
  const APP_BRANDING = globalThis.LOCAL_APP_BRANDING;
  if (!APP_BRANDING) return;
  'use strict';
  ...
  function emit(observation) {
    ...
    const takeUtf8 = (value, budget) => {
      ...
      const marker = '\n\n[Observação do navegador reduzida para respeitar o limite de transporte.]';
  ...
  function streamRow(entry, expandedTools) {
    ...
    if (entry.kind === 'tool_call') {
      body.textContent = entry.summary && entry.summary.title ? entry.summary.title : `Executou ${entry.tool || 'ferramenta'}`;
      ...
      body.textContent = entry.label || 'Ferramenta do ChatGPT';
    } else if (entry.kind === 'turn_start') {
      body.textContent = 'Turno iniciado';
   ...
   */
  const OUR_CONNECTORS = [APP_BRANDING.connectorNames.core, APP_BRANDING.connectorNames.desktop];
  ...
  function controlState(input) {
    ...
    if (phase === 'delivering') {
      return {
        mode: 'busy',
        label: NATIVE_PHASE_LABELS[phase] || 'Salvando…',
        hint: error || 'O resumo está pronto; aguardando o aplicativo salvá-lo.',
        action: 'cancel'
      };
    }
    ...
    if (job && job.busy && error && !phase) {
      ...
      return { mode: 'error', label: 'Pausado', hint: error, action: 'cancel' };
    }

    if (job && job.busy) {
      if (job.stage === 'opening') {
        return { mode: 'busy', label: 'Abrindo…', hint: 'Resumo salvo; abrindo a nova conversa', action: 'cancel' };
      }
      if (job.stage === 'waiting-for-browser') {
        return {
          mode: 'waiting',
          label: 'Aguardando…',
          hint: job.error || 'O aplicativo está tentando abrir a nova conversa.',
          action: 'cancel'
        };
      }
      ...
      return {
        mode: 'busy',
        label: NATIVE_PHASE_LABELS[phase] || 'Solicitando…',
        hint: 'O ChatGPT está escrevendo o resumo',
        action: 'cancel'
      };
    }
    /**
    ...
    if (job && job.stage === 'done') {
      return { mode: 'done', label: 'Aberto', hint: 'A nova conversa está aberta', action: 'start' };
    }
    if (job && job.stage === 'failed') {
      if (job.error === 'cancelled') {
        return { mode: 'idle', label: 'Compactar', hint: 'Retomada cancelada', action: 'start' };
      }
      return { mode: 'error', label: 'Falhou', hint: job.error || 'A compactação falhou', action: 'start' };
    }
    if (pressedAt > 0 && now - pressedAt < PRESS_GRACE_MS) {
      return { mode: 'busy', label: 'Iniciando…', hint: '', action: 'none' };
    }
    if (error) return { mode: 'error', label: 'Falhou', hint: error, action: 'start' };
    if (disconnected) {
      return {
        mode: 'off',
        label: 'Compactar',
        hint: 'A conexão com o navegador está desativada no aplicativo.',
        action: 'none'
      };
    }
    if (!connected) {
      return {
        mode: 'off',
        label: 'Compactar',
        hint: 'O aplicativo não está aberto neste computador.',
        action: 'none'
      };
    }
    if (!conversationId) {
      ...
      return {
        mode: 'off',
        label: 'Compactar',
        hint: 'Ainda não há o que compactar. Envie uma mensagem ou defina um objetivo para iniciar.',
        action: 'none'
      };
    }
    return { mode: 'idle', label: 'Compactar', hint: '', action: 'start' };
  }
  ...
  function settingsView(input) {
    ...
    const from = threshold > 0 ? `a partir de ${roundK(threshold)} tokens` : '';
    ...
    return {
      ...
      tip: [
        auto ? `Compactação automática ativada${from ? `, ${from}` : ''}` : 'Compactação automática desativada',
        blocked === 'worker'
          ? 'Objetivo desativado — o principal conduz esta conversa'
          : blocked === 'blocked'
            ? 'Objetivo desativado — conversa bloqueada no aplicativo'
            : fresh
            ? !hasKey
              ? 'Sem chave de API — Objetivo e Repetição indisponíveis'
              : objective
                ? 'Abrindo esta conversa com seu objetivo'
                : 'Adicione um objetivo ou repetição para iniciar'
            : position === 'off'
              ? 'Objetivo e Repetição desativados'
              : position === 'loop'
                ? hasKey
                  ? 'Repetição ativada — exige parada manual'
                  : 'Repetição ativada — sem chave de API'
                : hasKey
                  ? objective
                    ? 'Objetivo ativado — buscando o resultado desta conversa'
                    : 'Objetivo ativado'
                  : 'Objetivo ativado — sem chave de API'
      ].join('\n'),
      rows: [
        {
          key: 'autoCompact',
          ...
          note:
            blocked === 'worker'
              ? 'desativada: auxiliares não compactam automaticamente'
              : blocked === 'blocked'
                ? 'desativada: conversa bloqueada no aplicativo'
                : auto
                  ? from || 'limite definido no aplicativo'
                  : 'compacte esta conversa manualmente',
          on: auto,
          warn: false,
          disabled: fenced
        }
      ...
      mode: fresh
        ...
        : {
            value: position,
            options: [
              { value: 'off', label: 'Desativado', hint: 'Nenhuma mensagem é escrita automaticamente.' },
              {
                value: 'goal',
                label: 'Objetivo',
                hint: `Responde em seu nome até atingir o objetivo e então para. Usa ${modelLabel(goal && goal.model)}.`
              },
              {
                value: 'loop',
                label: 'Repetição',
                hint: `Responde em seu nome até você desativar esta opção. Usa ${modelLabel(goal && goal.model)}.`
              }
            ],
            ...
            note:
              blocked === 'worker'
                ? 'o principal conduz esta conversa'
                : blocked === 'blocked'
                  ? 'bloqueada no aplicativo'
                  : !hasKey
                  ? 'Chave do OpenRouter necessária'
                  : position === 'loop'
                    ? 'responde até você desativar'
                    : position === 'goal'
                      ? 'responde até atingir o objetivo'
          ...
          ? [
              {
                ...
                hint: 'Descreva o resultado desejado. A conversa continua até alcançá-lo e então para.'
              },
              {
                mode: 'loop',
                label: 'adicionar repetição',
                hint: 'Descreva o resultado desejado. A conversa continua até você desativar a Repetição.'
              }
            ]
          : [
              {
                ...
                // as a mode switch.
                mode: driving,
                label: objective ? 'editar tarefa' : 'adicionar tarefa',
                ...
                hint:
                  position === 'off'
                    ? 'Selecione Objetivo ou Repetição primeiro. Desativado não escreve mensagens.'
                    : objective
                      ? `Altere ou limpe o resultado desejado. O modo é ${driving === 'loop' ? 'Repetição' : 'Objetivo'}.`
                      : `Descreva o resultado desejado. O modo é ${driving === 'loop' ? 'Repetição' : 'Objetivo'}.`
              }
            ],
        available: hasKey && !blocked,
        unavailable:
          blocked === 'worker'
            ? 'A conversa auxiliar já é conduzida pelo principal.'
            : blocked === 'blocked'
              ? 'Esta conversa está bloqueada no aplicativo. Desbloqueie-a lá para continuar.'
              : !hasKey
              ? 'Adicione uma chave de API do OpenRouter no aplicativo primeiro.'
              : ''
      },
      ...
      action: {
        label:
          fenced
            ? 'Compactar e retomar indisponível'
            : compact.action === 'cancel'
              ? 'Cancelar compactação'
              : 'Compactar e retomar agora',
        hint:
          blocked === 'worker'
            ? 'Auxiliares permanecem na conversa atual e não podem ser compactados ou retomados manualmente.'
            : blocked === 'blocked'
              ? 'Conversas bloqueadas não são compactadas nem retomadas. Desbloqueie no aplicativo primeiro.'
              : compact.hint,
  ...
  function buildControl() {
    ...
    cancel.textContent = '×';
    cancel.setAttribute('aria-label', 'Cancelar compactação e retomada');
    ...
    blocked.textContent = 'Conversa bloqueada';
    blocked.setAttribute(
      'data-clf-tip',
      'Esta conversa está bloqueada no aplicativo: ferramentas, objetivo, repetição e compactação automática estão desativados. Para liberar, abra a lista de conversas no aplicativo e use o símbolo de bloqueio.'
    );
    blocked.hidden = true;
  ...
  function buildMenu() {
    ...
    root.setAttribute('aria-label', `Configurações de ${APP_BRANDING.shortName}`);
    root.hidden = true;
  ...
  function buildMode(mode) {
    ...
    track.setAttribute('aria-label', 'Modo de objetivo');
  ...
  function buildObjective(objective) {
    ...
    if (!objective.editing) {
      ...
      for (const action of objective.actions) {
        ...
        word.textContent = objectiveBusy ? 'processando…' : action.label;
    ...
    input.rows = 3;
    input.placeholder = 'Qual resultado esta conversa deve alcançar?';
    ...
    save.textContent = objectiveBusy ? 'Salvando…' : objective.mode === 'loop' ? 'Salvar como repetição' : 'Salvar como objetivo';
    ...
    if (objective.savable === false) {
      buttons.setAttribute('data-clf-tip', 'Selecione Objetivo ou Repetição primeiro. Desativado não escreve mensagens.');
    }
  ...
  function renderControl() {
    ...
    control.button.setAttribute('aria-label', `Configurações de ${APP_BRANDING.shortName}`);
  ...
  function foldBootstrap() {
    ...
    label.textContent =
      bootstrap === 'worker'
        ? `Este é ${bootstrapAgent || agent || 'um auxiliar'} — instrução enviada pelo aplicativo, não por você`
        : 'Resumo trazido pelo aplicativo para continuar o trabalho — não foi digitado por você';
    head.append(label);
   ...
   */
  const COMPACT_STEPS = ['Preparando', 'Escrevendo o resumo', 'Salvando', 'Abrindo nova conversa'];
  ...
  function stageView(input) {
    ...
    if (job && job.busy) {
      const stage =
        job.stage === 'opening'
          ? 'Abrindo uma nova conversa'
          : job.stage === 'waiting-for-browser'
            ? 'Aguardando o Chrome'
            : 'O ChatGPT está escrevendo o resumo';
   ...
   */
  const GOAL_STEPS = ['Concluindo resposta', 'Lendo conversa', 'Escrevendo resposta', 'Enviando'];
  ...
  function goalStageView(goal) {
    ...
    const failure = goal.error || (draft && draft.stage === 'failed' ? draft.error || 'O OpenRouter não respondeu' : '');
    if (failure) {
      ...
      if (goal.phase === 'retrying') {
        ...
        return { stage: `Nova tentativa do objetivo em ${seconds} segundos`, detail: failure, body: '', kind: 'goal', ...bar(at) };
      }
      return { stage: 'A continuidade por objetivo parou', detail: failure, body: '', kind: 'goal-error', ...bar(at) };
    }
    ...
    if (goal.opening) {
      if (goal.phase === 'sending') return { stage: 'Enviando ao ChatGPT', detail: '', body: '', kind: 'goal', ...bar(3) };
      return { stage: `${who} está escrevendo a primeira mensagem`, detail: '', body: '', kind: 'goal', ...bar(2) };
    }
    if (goal.phase === 'done') {
      ...
      return { stage: 'Objetivo alcançado', detail: 'nenhuma mensagem foi enviada', body: '', kind: 'goal-done', ...bar(2, true) };
    }
    if (goal.phase === 'settling') {
      return { stage: 'Verificando se a resposta terminou', detail: '', body: '', kind: 'goal', ...bar(0) };
    }
    if (goal.phase === 'sending' && draft && draft.reply) {
      return { stage: 'Enviando ao ChatGPT', detail: '', body: draft.reply, kind: 'goal', ...bar(3) };
    }
    if (goal.phase === 'requesting' && !draft) {
      return { stage: 'Enviando resposta ao OpenRouter', detail: who, body: '', kind: 'goal', ...bar(1) };
    }
    ...
    if (draft.stage === 'answering') {
      ...
      return { stage: `${who} está respondendo`, detail: '', body: draft.text || '', kind: 'goal', ...bar(2) };
    }
    if (draft.stage === 'ready') {
      // Written, not yet typed: the third segment is full and the fourth has not started.
      return { stage: `${who} escreveu a próxima mensagem`, detail: '', body: draft.reply || '', kind: 'goal', ...bar(2, true) };
    }
    return null;
  }
  ...
  function buildStage() {
    ...
    close.textContent = '×';
    close.title = 'Fechar';
    close.setAttribute('aria-label', 'Fechar estado do objetivo');
    close.hidden = true;
  ...
  async function maybeSendGoalReply() {
    ...
    if (draft.stage === 'failed') {
      goalDraft = null;
      const why = draft.error || 'O OpenRouter não respondeu';
      const pending = goalConfig && goalConfig.pending;
  ...
  function replyError(reply) {
    ...
    if (reply.error === 'app_not_found') return 'O aplicativo não está aberto neste computador.';

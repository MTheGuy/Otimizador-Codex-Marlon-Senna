...
**Core** (`${BRANDING.packageName}-core`, required):
...
**Desktop** (`${BRANDING.packageName}-desktop`, optional, **Windows/macOS**): `observe` needs `screen`;
...
**Estado local a inspecionar.** Use o caminho retornado por `app.getPath('userData')`,
que depende da plataforma e da identidade configurada. Ele contém `config.json`
(configurações validadas sem segredos), `sessions/` (histórico durável),
...
Identidade e nome do produto vêm de `customization/app.config.json`, expostos por `BRANDING`.
Esta distribuição entrega Windows x64 NSIS. As receitas Windows ARM64, macOS x64/ARM64
DMG+ZIP e Linux x64/ARM64 AppImage+DEB permanecem disponíveis, sem validação nesta entrega.
Windows mantém instalação por usuário, `asInvoker` e nenhuma elevação obrigatória.

...
export function createAgentPanel(options: {
  ...
  pane.setAttribute('aria-label', 'Subagentes');
  const head = el('div', 'agent-panel-header');
  const back = el('button', 'btn', '←'); back.title = 'Voltar aos subagentes'; back.setAttribute('type', 'button');
  back.setAttribute('aria-label', back.title);
  const title = el('strong', '', 'Subagentes');
  const close = el('button', 'btn', '×'); close.setAttribute('type', 'button'); close.setAttribute('aria-label', 'Fechar subagentes');
  ...
  function list(): void {
    generation++; selected = null; back.hidden = true; title.textContent = 'Subagentes'; body.replaceChildren();
    for (const active of [true, false]) {
      const group = workers.filter(worker => options.working(worker) === active);
      body.append(el('h3', '', `${active ? 'Ativos' : 'Histórico'} · ${group.length}`));
      if (!group.length) { body.append(el('p', 'meta', active ? 'Nenhum subagente ativo' : 'Nenhum subagente registrado')); continue; }
  ...
  async function open(id: string, refresh = false): Promise<void> {
    ...
    if (!preserve) body.replaceChildren(el('p', 'meta', 'Carregando conversa…'));
    const current = () => request === generation && selected === id && !pane.hidden;
    const detail = await options.load(id);
    if (!current()) return;
    if (!detail) { body.replaceChildren(el('p', 'meta', 'Conversa indisponível')); return; }
    const openMain = el('button', 'btn', 'Abrir conversa completa'); openMain.setAttribute('type', 'button');
  ...
  return {
    ...
    update(id: string | null, next: SessionSummary[]): void {
      ...
      options.toggle.title = `Subagentes · ${workers.length} registrados`;

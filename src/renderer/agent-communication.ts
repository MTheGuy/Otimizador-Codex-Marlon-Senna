...
export function communicationTitle(event: Communication): string {
  ...
  if (event.from === 'prime') return `Mensagem para ${worker}`;
  if ((event.message.text.startsWith(`[${worker} is awake again]`) || event.message.text.startsWith(`[${worker} is back]`))) return `${worker} retomou o trabalho`;
  if ((event.message.text.startsWith(`[${worker} reported]`) || event.message.text.startsWith(`[${worker} finished]`))) return `${worker} concluiu · relatório`;
  return `Mensagem de ${worker}`;
}

...
export function paintContextMeter(session: SessionSummary | null, config: Config, composer: { model: string; reasoningEffort: ReasoningEffort } | null = null): void {
  ...
  panel.textContent = pro
    ? `Contexto da sessão · estimativa\n${tokens} tokens usados\nCompactação automática desativada no Pro`
    : `Contexto da sessão · estimativa\n${tokens} / ${new Intl.NumberFormat().format(limit)} tokens · ${percent}% do limite configurado\n${config.compaction.auto ? `Compactação automática em ${new Intl.NumberFormat().format(config.compaction.autoTokens)} tokens` : 'Compactação automática desativada'}`;

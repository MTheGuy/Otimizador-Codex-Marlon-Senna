...
import { SECURITY_PROFILE_DETAILS, SECURITY_PROFILE_LABELS, securityPreset, securityProfileFor,
  type SecurityProfile } from '../shared/security-profiles.js';
...
const GROUPS: Group[] = [
  {
    id: 'read',
    title: 'Ler arquivos dos projetos',
    icon: 'i-eye',
    blurb: 'Ler e pesquisar somente nas pastas que você aprovou.',
    caps: ['browse', 'search', 'read', 'metadata']
  },
  {
    id: 'write',
    title: 'Modificar arquivos dos projetos',
    icon: 'i-pencil',
    blurb: 'Criar, editar, mover e excluir somente nas pastas aprovadas.',
    caps: ['create', 'edit', 'move', 'deleteFile']
  },
  {
    id: 'screen',
    title: 'Observar a tela',
    icon: 'i-monitor',
    blurb: 'Ver a tela e as janelas, inclusive fora dos projetos.',
    caps: ['screen']
  },
  {
    id: 'control', title: 'Controlar mouse e teclado', icon: 'i-monitor',
    blurb: 'Operar aplicativos com o acesso da sua conta no computador.', caps: ['control']
  },
  {
    id: 'clipboardRead', title: 'Ler área de transferência', icon: 'i-copy',
    blurb: 'Ler o texto copiado no sistema, inclusive de outros aplicativos.', caps: ['clipboardRead']
  },
  {
    id: 'clipboardWrite', title: 'Escrever na área de transferência', icon: 'i-copy',
    blurb: 'Substituir o texto copiado no sistema.', caps: ['clipboardWrite']
  },
  {
    id: 'run',
    title: 'Executar comandos',
    icon: 'i-terminal',
    blurb: 'Executar programas com o acesso da sua conta, não limitado aos projetos.',
    caps: ['command']
  }
];
...
let securityProfileDraft: SecurityProfile | null = null;
...
function buildGroups(): void {
  const permissionGroups = GROUPS.map((group) => {
    ...
    box.title = `Ativar ou desativar: ${group.title}`;
  ...
  record.title = 'Salvar conversas localmente para consulta, continuidade e coordenação de agentes';
  record.addEventListener('change', () => void save());
  const recording = groupShell('recording', 'Histórico local de conversas', 'i-steps', record);
  const recordTools = el('div', 'tools');
  for (const [name, detail] of [
    ['Pesquisar', 'Localizar trabalhos anteriores ou em andamento no histórico local.'],
    ['Consultar', 'Ler uma conversa específica, continuar seu trabalho e consultar os detalhes das ferramentas.']
  ...
  enabled.title = 'Disponibilizar ferramentas de coordenação de outros agentes';
  ...
  const agents = groupShell('agents', 'Subagentes', 'i-bolt', enabled);

  const tools = el('div', 'tools');
  const agentTools: Array<[string, string]> = [
    ['Criar', 'Abrir agentes para partes independentes da tarefa.'],
    ['Conversar', 'Enviar orientações e resultados entre agentes identificados.'],
    ['Acompanhar', 'Consultar o estado dos agentes e receber mensagens pendentes.'],
    ['Concluir', 'Entregar o resultado ao agente coordenador.']
  ];
...
function buildSecurityControls(): void {
  const select = $<HTMLSelectElement>('securityProfile');
  for (const [value, label] of Object.entries(SECURITY_PROFILE_LABELS)) {
  ...
  select.addEventListener('change', () => {
    securityProfileDraft = select.value as SecurityProfile;
    $('securityProfileDetail').textContent = SECURITY_PROFILE_DETAILS[securityProfileDraft];
  });
  $('securityApply').addEventListener('click', () => {
    const preset = securityPreset(select.value as SecurityProfile);
    if (!preset) {
      openGroup = 'read';
      paintGroups();
      toast('Personalize as permissões abaixo. Somente leitura precisa ser desativado para permitir alterações.');
      return;
    }
    securityProfileDraft = null;
    void save({ ...preset, reviewed: true });
  });
  $('securityConfirm').addEventListener('click', () => void save({ reviewed: true }));
}

function paintSecurity(next: AppState): void {
  const profile = securityProfileFor(next.config);
  const selected = securityProfileDraft ?? profile;
  $<HTMLSelectElement>('securityProfile').value = selected;
  $('securityProfileDetail').textContent = SECURITY_PROFILE_DETAILS[selected];
  $('securityReview').hidden = next.config.security?.reviewed === true;
  $('securityCurrent').textContent = `Perfil atual: ${SECURITY_PROFILE_LABELS[profile]}. ` +
    (next.config.readOnly ? 'Alterações bloqueadas por somente leitura.' : 'Valem as permissões ativas abaixo.');
}
...
function paintGroups(): void {
  ...
  for (const group of GROUPS) {
    ...
    const supported = !group.caps.every(cap => DESKTOP_CAPABILITIES.includes(cap)) || desktopSupported;
    ...
    const on = usable.filter((cap) => capInput(cap).checked);
      ...
      usable.length === 0
        ...
        : on.length === 0
          ...
          : on.length === group.caps.length
            ? `${on.length} ${on.length === 1 ? 'permissão ativa' : 'permissões ativas'}`
            : `${on.length} de ${group.caps.length} permissões ativas`;
  ...
  for (const cap of WRITE_CAPABILITIES)
    capInput(cap).disabled = readOnly || (!desktopSupported && DESKTOP_CAPABILITIES.includes(cap));
  ...
  for (const [id, onText] of [
    ['recording', 'ferramentas de sessão disponíveis'],
    ['agents', 'coordenação de agentes disponível']
  ] as Array<[string, string]>) {
    ...
    root.querySelector<HTMLElement>('.group-count')!.textContent = box.checked ? onText : 'desativado';
  }
}
...
function paintDesktopAccess(next: AppState): void {
  ...
  if (needsScreen && access.screen !== 'granted') missing.push(`Gravação da Tela: ${access.screen}`);
  if (needsAccessibility && access.accessibility !== 'granted') {
    missing.push(`Acessibilidade: ${access.accessibility}`);
  }
  box.hidden = missing.length === 0;
  if (box.hidden) return;

  $('desktopAccessTitle').textContent = 'O acesso ao computador precisa de atenção';
  $('desktopAccessDetail').textContent =
    `${missing.join(' · ')}. Resultado atual do componente nativo de ${BRANDING.name}. ` +
    'Conceda a permissão indicada no macOS, encerre completamente o aplicativo e abra-o novamente.';
...
function save(over: { readOnly?: boolean; theme?: 'light' | 'dark'; reviewed?: boolean;
  capabilities?: AppState['config']['capabilities'] } = {}): Promise<void> {
  ...
  const patch: SettingsPatch = {
    ...
    readOnly,
    security: { reviewed: over.reviewed ?? previous.security?.reviewed ?? false },
    tunnel: {
    ...
    ui: {
      displayName: $<HTMLInputElement>('displayName').value.trim() || BRANDING.name,
...
async function saveSnapshot(patch: SettingsPatch, previous: AppState['config']): Promise<void> {
  ...
  const base: SettingsPatch = {
    ...
    security: previous.security ?? { reviewed: false },
  ...
  if (next) {
    ...
    if (previous.multiAgent.enabled && !patch.multiAgent.enabled) {
      ...
      toast('Subagentes desativados. Reconecte o conector no ChatGPT e abra uma nova conversa para atualizar as ferramentas.');
    } else if (toolSurfaceChanged) {
      toast('Permissões atualizadas. Inicie uma nova conversa no ChatGPT para atualizar a lista de ferramentas.');
    }
  } else await refresh();
...
const STATUS_TEXT: Record<AppState['status']['state'], string> = {
  ...
  'starting-server': 'Iniciando',
  'connecting-tunnel': 'Conectando',
  connected: 'Conectado',
  offline: 'Sem internet',
  'auth-failed': 'Falha na autenticação',
  'tunnel-unavailable': 'Túnel indisponível'
};

const METHOD_HINT: Record<string, string> = {
  openai:
    'O ChatGPT acessa este computador por um túnel OpenAI, sem expor um endereço à internet aberta.',
  cloudflared:
    'Cria um endereço HTTPS público temporário com a Cloudflare. O endereço muda a cada reinicialização.',
  manual: 'O aplicativo aceita conexões somente em localhost. Você é responsável por disponibilizar o túnel.'
};
...
function missingStep(next: AppState): { step: string; text: string } | null {
  ...
  if (config.roots.length === 0 && requiresApprovedFilesystemRoot(config)) {
    return { step: 'folder', text: 'Escolha um projeto para compartilhar — etapa 1.' };
  }
  if (config.tunnel.kind === 'openai') {
    if (!TUNNEL_ID_PATTERN.test(config.tunnel.tunnelId)) {
      return { step: 'tunnel', text: 'Crie um túnel e cole o identificador — etapa 2.' };
    }
    if (!(next.secureStorage?.available ?? true) && !next.hasApiKey) {
      return { step: 'key', text: next.secureStorage?.detail ?? 'O armazenamento seguro de credenciais está indisponível.' };
    }
    if (!next.hasApiKey) {
      return { step: 'key', text: 'Adicione uma chave de API restrita — etapa 3.' };
    }
  } else if (!next.resolvedBinary && config.tunnel.kind === 'cloudflared') {
    return { step: 'connect', text: 'O cloudflared não foi encontrado neste computador.' };
  }
  return null;
}
...
function rootRow(root: AppState['config']['roots'][number]): HTMLElement {
  ...
  if (renameState) {
    ...
    input.setAttribute('aria-label', `Renomear /${root.name}`);
  ...
  rename.type = 'button';
  rename.title = `Renomear /${root.name}`;
  ...
  remove.type = 'button';
  remove.title = `Deixar de compartilhar /${root.name}`;
...
function apply(next: AppState): void {
  ...
  const displayName = config.ui.displayName?.trim() || BRANDING.name;
  document.title = displayName;
  for (const label of document.querySelectorAll<HTMLElement>('[data-app-name]')) label.textContent = displayName;
  applyValue($<HTMLInputElement>('displayName'), displayName, previousState?.config.ui.displayName);
  ...
  $('connectLabel').textContent = running ? 'Desconectar' : 'Conectar';
  ...
  if (next.platform?.family === 'macos') {
    $('backgroundRunningCopy').textContent =
      'Deixe o aplicativo em execução enquanto usa o conector. Ao fechar a janela, ele continua na barra de menus e no Dock.';
    $('minimizeToTrayCopy').textContent = 'Ocultar na barra de menus ao fechar a janela';
  } else {
    $('backgroundRunningCopy').textContent =
      'Deixe o aplicativo em execução enquanto usa o conector. Ao fechar a janela, ele continua na bandeja do sistema.';
    $('minimizeToTrayCopy').textContent = 'Continuar na bandeja do sistema ao fechar';
  }
  ...
  $('wizFolders').textContent =
    config.roots.length === 0 ? 'Nenhum ainda' : config.roots.map((r) => `/${r.name}`).join('  ');
  const secureStorageAvailable = next.secureStorage?.available ?? true;
  const apiKey = $<HTMLInputElement>('apiKey');
  apiKey.placeholder = next.hasApiKey ? '•••••••• salva' : 'sk-…';
  apiKey.disabled = !secureStorageAvailable;
  $('apiKeyState').textContent = !secureStorageAvailable
    ? (next.secureStorage?.detail ?? 'O armazenamento seguro de credenciais está indisponível.')
    : next.hasApiKey
      ? 'Há uma chave no armazenamento seguro do sistema operacional. Digite outra para substituí-la ou use Remover chave de API salva.'
      : 'Salva no armazenamento seguro do sistema. Não será exibida novamente nem enviada à interface.';
  ...
  wizConnect.textContent = running ? 'Desconectar' : 'Conectar';
  ...
  $('chatgptConn').replaceChildren(
    openai
      ? frag('Na conexão, escolha ', 'Tunnel', ' e selecione o túnel criado na etapa 2.')
      : frag('Na conexão, cole o endereço abaixo em ', 'URL do servidor MCP', '.')
  );
  ...
  chatgptNote.textContent =
    status.lastRequestAt === null
      ? 'O ChatGPT ainda não chamou este aplicativo.'
      : status.lastToolCallAt === null
        ? `O ChatGPT conectou ${ago(status.lastRequestAt)}, mas ainda não executou ferramentas. Se informar que não aceita MCPs de desenvolvedor, reative o Modo de desenvolvedor em ChatGPT → Configurações → Aplicativos e conectores → Avançado.`
        : unverified.length > 0
          ? // One connector working is not the whole setup. Naming the missing one is the
            ...
            `O ChatGPT executou uma ferramenta ${ago(status.lastToolCallAt)}, mas ${unverified
              .map((surface) => `“${surface.connectorName}”`)
              .join(' e ')} ainda não recebeu chamadas — crie esse conector no ChatGPT para utilizá-lo.`
          : `O ChatGPT executou uma ferramenta ${ago(status.lastToolCallAt)} — o fluxo completo está funcionando.`;
  ...
  expand.textContent = showAllSteps ? 'Ocultar etapas concluídas' : 'Mostrar todas as etapas';

  const needsBinary = config.tunnel.kind !== 'manual';
  $('binaryState').textContent = !needsBinary
    ? 'Desnecessário para este método.'
    : next.resolvedBinary
      ? `Em uso: ${next.resolvedBinary}`
      : 'Não encontrado. Instale o programa ou selecione o arquivo em Procurar.';
  $('versionLine').textContent = next.bundledTunnelVersion
    ? `Somente atividade recente — sem conteúdo de arquivos ou credenciais. tunnel-client incluído: ${next.bundledTunnelVersion}.`
    : 'Somente atividade recente. Conteúdo de arquivos e credenciais não são registrados.';
...
function copyRow(label: string, value: string, what: string): HTMLElement {
  ...
  button.append(icon('i-copy'), document.createTextNode('Copiar'));
  button.addEventListener('click', async () => {
    const copied = await run(api.writeClipboard(value));
    if (copied) toast(`${what}: copiado`);
  });
...
function connectorCards(next: AppState): HTMLElement[] {
  ...
  return status.surfaces
    ...
    head.append(
      ...
      el('span', 'tag', surface.optional ? 'opcional' : 'necessário'),
    ...
    card.append(copyRow('Nome', surface.connectorName, 'Nome'));
    card.append(copyRow('Descrição', surface.description, 'Descrição'));
    ...
    if (url) {
      card.append(copyRow('URL do servidor MCP', url, 'URL'));
      card.append(
        el('p', 'hint', 'Qualquer pessoa com este endereço pode usar as ferramentas habilitadas. Não o compartilhe.')
      );
      ...
      card.append(
        el(
          ...
          surface.id === 'desktop' && !config.tunnel.desktopTunnelId
            ? 'Escolha o túnel deste conector — primeiro cole o identificador na etapa 2.'
            : 'Escolha Tunnel e depois selecione o túnel deste conector.'
        )
      );
    }
    ...
    if (surface.state === 'live') {
      card.append(
        surface.lastRequestAt === null
          ? el('p', 'hint is-warn', 'Ainda não utilizado no ChatGPT — esse conector nunca recebeu chamadas.')
          : el(
              'p',
              'hint',
              surface.lastToolCallAt === null
                ? `O ChatGPT conectou ${ago(surface.lastRequestAt)}, mas ainda não executou as ferramentas deste conector.`
                : `O ChatGPT executou uma ferramenta deste conector ${ago(surface.lastToolCallAt)}.`
            )
      );
    }

    if (surface.tools.length > 0) {
      card.append(el('p', 'hint', `Ferramentas: ${surface.tools.join(', ')}`));
    }
    return card;
    });
}
...
function facts(next: AppState): HTMLElement[] {
  ...
  if (isRunning(status.state)) {
    rows.push(['Conexão com OpenAI', health?.route ?? 'iniciando…']);
    rows.push([
      'Falhas de consulta',
    ...
    rows.push(['Tempo ativo do túnel', duration(health?.uptimeSeconds ?? null)]);
    ...
    if (status.lastRequestAt !== null) {
      rows.push([
        'Ferramenta executada pelo ChatGPT',
        status.lastToolCallAt === null ? 'nunca — confira o Modo de desenvolvedor' : ago(status.lastToolCallAt),
        status.lastToolCallAt === null
      ]);
    }
    if (health?.clientVersion) rows.push(['Cliente do túnel', health.clientVersion]);
    if (status.localUrl) rows.push(['Servidor local', status.localUrl.replace(/^https?:\/\//, '')]);
  } else {
    rows.push(['Conexão com OpenAI', 'inativo']);
  }

  rows.push([
    'Ferramentas do conector principal e computador',
    `${toolsOn(next)} total · ${config.roots.length} projeto${config.roots.length === 1 ? '' : 's'}`
  ]);
...
function paintClock(): void {
  ...
  $('liveNote').textContent = running
    ? status.handshakeAt === null
      ? 'conexão ainda não verificada'
      : `verificado ${ago(status.handshakeAt)}`
    : '';
}
...
function paintProblems(): void {
  ...
  for (const id of ['homeProblems', 'logProblems']) {
    ...
    badge.textContent = `${problems} problema${problems === 1 ? '' : 's'}`;
  }
}
...
function paintAgentFilter(swarm: SwarmState): void {
  ...
  const choices: Array<{ id: string | null; label: string }> = [{ id: null, label: 'Todos' }];
...
async function runChecks(): Promise<void> {
  ...
  $('runChecksLabel').textContent = 'Verificando…';
  try {
    ...
    $('runChecksLabel').textContent = 'Executar verificações';
  }
}
...
$('displayName').addEventListener('change', () => void save());
$('resetDisplayName').addEventListener('click', () => {
  $<HTMLInputElement>('displayName').value = BRANDING.name;
  void save();
});
...
for (const id of ['copyLog', 'copyLogText']) {
  $(id).addEventListener('click', async () => {
    ...
    if (copied) toast('Atividade copiada');
  });
}
...
$('copyLogJson').addEventListener('click', async () => {
  ...
  if (text === null) return;
  const copied = await run(api.writeClipboard(text));
  if (copied) toast('JSON da atividade copiado');
});
...
for (const label of document.querySelectorAll<HTMLElement>('[data-app-name]')) label.textContent = BRANDING.name;
buildGroups();
...
void (async () => {
  ...
  showTab(initial && !initial.config.security?.reviewed ? 'home' :
    initial && missingStep(initial)?.step === 'folder' ? 'setup' : 'chat');

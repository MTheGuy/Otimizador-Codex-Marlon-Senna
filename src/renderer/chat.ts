...
function paintComposerImages(): void {
  ...
  images.forEach((image, index) => {
    ...
    const remove = el('button', 'image-remove', '×'); remove.setAttribute('type', 'button'); remove.setAttribute('aria-label', `Remover ${image.name}`);
...
const AGENT_BADGE: Record<AgentState, Badge> = {
  invited: { text: 'abrindo', tone: 'is-active' },
  active: { text: 'ativo', tone: 'is-active' },
  ...
  detached: { text: 'sem aba', tone: 'is-active' },
  ...
  sleeping: { text: 'em espera', tone: '' },
  waking: { text: 'retomando', tone: 'is-active' },
  finished: { text: 'concluído', tone: 'is-finished' },
  failed: { text: 'falhou', tone: 'is-failed' }
};
...
/** Ausência de atribuição nunca concede autoridade para executar ferramentas. */
...
function sessionBadges(summary: SessionSummary): Badge[] {
  ...
  if (summary.conversationId === null) {
    return unattributedBlocked()
      ? [{ text: 'bloqueado', tone: 'is-failed' }, { text: 'não é uma conversa', tone: '' }]
      : [{ text: 'não é uma conversa', tone: '' }];
  }
  ...
  if (blockedChats.has(summary.conversationId)) badges.push({ text: 'bloqueado', tone: 'is-failed' });
  if (origin?.kind === 'worker') badges.push({ text: origin.agentId ?? 'executor', tone: '' });
  else if (origin?.kind === 'resume') badges.push({ text: 'retomado', tone: '' });
  else if (summary.agents.includes('prime')) badges.push({ text: 'principal', tone: '' });
...
function sessionRow(summary: SessionSummary): HTMLElement {
  ...
  const title = el('b', '', summary.title || 'Conversa sem título');
  top.append(title);
  const badges = sessionBadges(summary);
  row.title = [summary.title || 'Conversa sem título', ...badges.map((badge) => badge.text), ago(summary.updatedAt)].join(' · ');
  const showTip = () => {
  ...
  remove.type = 'button';
  remove.title = 'Excluir esta conversa gravada';
  ...
  if (summary.conversationId === null) {
    // Sem conversa comprovada, não existe uma ação de interface capaz de conceder autoridade.
  ...
  if (summary.conversationId) {
    ...
    block.type = 'button';
    block.title = blocked
      ? 'Liberar esta conversa: suas ferramentas voltam a executar'
      : 'Bloquear esta conversa: recusar suas chamadas de ferramentas e solicitar que pare';
    ...
    open.type = 'button';
    open.title = 'Abrir esta conversa no Chrome';
...
function paintSessions(): void {
  ...
  const group = (key: string, workers: SessionSummary[], parentRow?: HTMLElement, target = rows): void => {
    ...
    button.title = `${workers.length} subagentes · ${workers.filter(sessionWorking).length} ativos`;
    button.setAttribute('aria-label', `${expandedWorkers.has(key) ? 'Recolher' : 'Expandir'} ${workers.length} subagentes`);
  ...
  if (otherWorkers.length) {
    ...
    history.append(el('summary', '', `Histórico de subagentes · ${otherWorkers.length}`));
  ...
  for (const id of projectIds) {
    ...
    const label = el('span', 'project-name', project?.name ?? 'Projeto indisponível');
    heading.title = project?.path ?? 'Projeto indisponível';
    ...
    if (project) {
      ...
      create.title = 'Nova conversa neste projeto'; create.setAttribute('aria-label', create.title);
    ...
    if (shown.length < tasks.length) {
      const more = el('button', 'btn project-show-more', 'Mostrar mais') as HTMLButtonElement;
      more.type = 'button'; more.setAttribute('aria-label', `Mostrar mais tarefas em ${project?.name ?? 'este projeto'}`);
  ...
  if (diagnostics.length) {
    ...
    disclosure.append(el('summary', '', `Atividade sem atribuição · ${diagnostics.length}`));
  ...
  const retained = `${sessionTotal} ${sessionTotal === 1 ? 'conversa mantida' : 'conversas mantidas'}`;
  const shown = sessions.length < sessionTotal ? `${sessions.length} exibidas de ${retained}` : retained;
  const more = sessionPageCursor && sessions.length < sessionTotal ? ' · role para ver o histórico anterior' : '';
  $('sessionsFoot').textContent = recording
    ? `${shown}${more}${activeId ? ' · uma ativa agora' : ''}`
    : `Gravação desativada · ${shown}${more}`;
...
function paintGoalProgress(): void {
  ...
  const labels: Record<string, string> = { saving: 'Salvando tarefa…', saved: 'Tarefa salva · aguardando a próxima resposta concluída',
    preparing: 'Preparando a mensagem inicial…', generating: 'Gerando a mensagem inicial…', ready: 'Mensagem pronta · aguardando envio ao ChatGPT',
    sending: 'Preparando a continuação…', answering: 'Gerando a continuação…', queued: 'Mensagem inicial na fila',
    browser: 'Enviando mensagem inicial ao ChatGPT…', sent: 'Mensagem inicial enviada', tool: 'Mensagem inicial entregue ao turno ativo',
    failed: 'Não foi possível continuar a tarefa', cancelled: 'Mensagem inicial cancelada', paused: 'Automação pausada · texto da tarefa preservado', 'no-reply': 'Objetivo alcançado' };
  if (phase === 'retrying') { text = ''; error = undefined; }
  labels.retrying = `Provedor ocupado · nova tentativa ${progress?.attempt ?? ''}${progress?.retryAt ? ' às ' + new Date(progress.retryAt).toLocaleTimeString() : ''}`;
...
function paintDeliveryControls(): void {
  ...
  $('afterTurnLabel').textContent = queueAtFinish ? 'Colocar na fila ao encerrar a sessão' : 'Após este turno';
  ...
  if (sendOption) sendOption.textContent = canInject ? 'Enviar agora' : 'Enviar';
  ...
  send.setAttribute('aria-label', stop ? (controlledStopPending ? 'Parada solicitada' : 'Parar turno') : 'Enviar mensagem');
  if (stop && !working && pending) send.setAttribute('aria-label', 'Cancelar envio');
  if (preparedPlan && !stop) send.setAttribute('aria-label', 'Enviar primeira etapa');
  send.classList.toggle('is-plan-ready', !!preparedPlan && !stop);
  send.title = stop && !working && pending ? 'Cancelar envio' : preparedPlan && !stop ? 'Enviar primeira etapa' : '';
...
function paintActiveGoal(): void {
  ...
  const label = el('span', 'queue-label', `${mode === 'loop' ? 'Repetição' : 'Buscando o objetivo'}${objective ? ' · ' + objective : ''}`);
  label.title = objective;
  row.replaceChildren(icon('i-pulse'), label,
    dockAction('Pausar automação', 'i-power', () => { const select = $<HTMLSelectElement>('chatAutomation'); select.value = 'off'; select.dispatchEvent(new Event('change')); }),
    dockAction('Editar tarefa', 'i-pencil', () => { $<HTMLDetailsElement>('composerSettings').open = true; $<HTMLTextAreaElement>('sessionObjective').focus(); }));
}
...
function cancelTaskPlan(): void {
  ...
  $<HTMLTextAreaElement>('chatInput').placeholder = 'Escreva sua mensagem…';
  paintTaskActions();
  paintDeliveryControls();
}
async function createTaskPlan(backend: 'api' | 'chatgpt'): Promise<void> {
  ...
  if (!text) { input.placeholder = 'Descreva a tarefa que será transformada em plano…'; input.focus(); return; }
  const generation = ++planGeneration, selection = selectionGeneration;
  const preview = $('taskPlanPreview'); preview.hidden = false;
  preview.replaceChildren(el('span', 'muted', 'Criando plano…'));
  const button = $<HTMLButtonElement>('createPlan');
  const caption = button.querySelector('span')!; const previous = caption.textContent;
  button.dataset.busy = 'true'; button.setAttribute('aria-busy', 'true'); caption.textContent = 'Criando plano…'; paintTaskActions();
  ...
  const unsubscribe = api.onTaskProgress?.(progress => {
    ...
    const label = progress.phase === 'retrying' ? `Provedor ocupado · nova tentativa ${progress.attempt ?? ''}${progress.retryAt ? ' às ' + new Date(progress.retryAt).toLocaleTimeString() : ''}` : progress.phase === 'cancelled' ? 'Plano cancelado' : progress.phase === 'preparing' ? 'Preparando plano…' : progress.phase === 'ready' ? 'Plano pronto' : progress.phase === 'failed' ? 'Falha no plano' : 'Escrevendo plano…';
  ...
  try {
    ...
    if (!result.ok && generation === planGeneration && selection === selectionGeneration) {
      const error = el('div', 'muted', result.error === 'invalid_goal_decision_json' ? 'Não foi possível ler a resposta do planejador.' : result.error);
  ...
  if (!stages) { preview.hidden = false; preview.append(el('div', 'muted', 'Envie novamente para tentar outra vez ou cancele o plano.')); return; }
  if (input.value.trim() !== text) { preview.hidden = true; toast('Seu rascunho mudou. Gere um plano a partir da tarefa atualizada.'); return; }
...
function paintPreparedPlan(): void {
  ...
  preview.replaceChildren(...plan.stages.map((stage, index) => {
    ...
    const label = el('span', 'stage-number', String(index + 1)); label.setAttribute('aria-label', `Etapa ${index + 1}`);
    ...
    field.setAttribute('aria-label', `Editar etapa ${index + 1}`);
    const error = el('span', 'stage-error', 'Preencha o texto ou exclua esta etapa.'); error.id = `planStageError-${index}`; error.hidden = !!stage.trim();
    ...
    heading.title = index === 0 ? 'Enviar inicia esta etapa. As próximas chegam uma por vez ao encerrar a sessão ou após uma resposta concluída.' : 'Entregue ao encerrar a sessão ou após uma resposta concluída, depois da etapa anterior.';
...
async function sendPreparedPlan(): Promise<void> {
  ...
  if (!tasks.length || tasks.some(task => !task) || JSON.stringify(tasks).length > 12000) { toast('Preencha todas as etapas e mantenha o plano abaixo de 12.000 caracteres.'); return; }
...
function paintTaskActions(): void {
  ...
  save.querySelector('span')!.textContent = save.dataset.busy === 'true' ? 'Salvando…' : saved ? 'Salvo' : 'Salvar tarefa';
  ...
  for (const id of ['createPlan']) {
    ...
    button.querySelector('span')!.textContent = planMode ? 'Cancelar plano' : 'Criar plano';
    button.title = planMode ? 'Voltar à mensagem normal e manter o rascunho' : text ? 'Dividir sua mensagem em etapas editáveis' : 'Primeiro escreva uma mensagem';
  }
}
function paintAutomationSwitch(): void {
  ...
  document.querySelector('label[for="sessionObjective"]')!.textContent = loop ? 'Instruções de repetição' : 'Objetivo';
  $<HTMLTextAreaElement>('sessionObjective').placeholder = loop ? 'Qual deve ser o foco de cada continuação?' : 'O que esta conversa deve realizar?';
  paintTaskActions();
}
async function refreshSessionControls(): Promise<void> {
  ...
  $('sessionControlStatus').textContent = controls.blocked === 'worker' ? 'Este subagente é gerenciado pelo agente principal.' : controls.blocked === 'blocked' ? 'Esta conversa está bloqueada.' : controls.job?.busy ? 'A compactação está em andamento no ChatGPT.' : '';
}
...
function textBlock(className: string, value: string, truncated: boolean, chars: number): HTMLElement {
  ...
  if (truncated) {
    node.append(el('span', 'cut', ` … abreviado, ${compactNumber(chars)} caracteres no original`));
  }
  return node;
}
...
export function renderedMessage(html: StoredText | null | undefined, fallback: string): HTMLElement {
  ...
  for (const table of box.querySelectorAll('table')) {
    ...
    viewport.setAttribute('aria-label', 'Tabela');
...
function toolBody(event: Extract<SessionEvent, { kind: 'tool_call' }>): HTMLElement {
  ...
  facts.textContent =
    ...
    `atribuído por ${ATTRIBUTION_LABELS[call.attribution] ?? call.attribution}`;
  raw.append(facts);
  ...
  if (call.changes && call.changes.length > 0) {
    ...
    for (const change of call.changes) {
      ...
      const counts = `+${change.added} −${change.removed}${change.approximate ? ' (aprox.)' : ''}`;
...
function eventBody(event: SessionEvent, context?: { id: string; current: () => boolean }): HTMLElement {
  switch (event.kind) {
    case 'session_start':
      return el('p', 'meta', `Conversa iniciada — ${event.title}`);
    case 'user_message': {
      const box = el('div', 'said is-user');
      box.append(el('b', '', 'Você'));
      ...
      if (event.inputDelivery) {
        ...
        const label = event.inputDelivery === 'offered' ? 'Enviado ao turno ativo · aguardando confirmação' : 'Entrega confirmada';
      ...
      if (assets.length && (context?.id || selectedId)) {
        ...
        void (async () => {
          for (const asset of assets) {
            ...
            if (!data) { attachments.append(el('p', 'meta', 'Imagem indisponível')); continue; }
            const image = document.createElement('img');
            image.src = data; image.alt = 'Anexo do usuário'; image.loading = 'lazy';
    ...
    case 'assistant_message': {
      ...
      box.append(el('b', '', event.final ? 'ChatGPT' : 'ChatGPT (parcial)'));
    ...
    case 'turn_start':
      return el('p', 'meta', event.detail ? `Turno reaberto — ${event.detail}` : 'Turno iniciado');
    case 'turn_end': {
      const line = el(
        'p',
        event.outcome === 'completed' ? 'meta' : 'meta is-warn',
        `Turno ${TURN_OUTCOME_LABELS[event.outcome]}${event.detail ? ` — ${event.detail}` : ''}`
      );
      return line;
    }
    case 'chat_error': {
      ...
      const title = el('strong', '', 'O ChatGPT informou um problema');
    ...
    /**
     * Rendered rather than left to fall through to "Evento desconhecido".
     *
     * The timeline is how the user checks what the agents actually said to each other, and
     * a run of grey "Evento desconhecido" rows in the middle of a multi-agent session reads as a
    ...
    case 'agent_message': {
      ...
      box.title =
        event.delivery === 'sent'
          ? `Enviado por ${event.from}; registrado quando o aplicativo aceitou`
          : `Recebido por ${event.to}; registrado na confirmação da entrega`;
      ...
      if (!context) {
        ...
        if (matches.length === 1) {
          const open = el('button', 'btn', 'Abrir conversa do subagente'); open.setAttribute('type', 'button');
    ...
    case 'handoff':
      return el(
        'p',
        'meta is-good',
        `Resumo salvo — ${compactNumber(event.chars)} caracteres (${event.reason})`
      );
    default:
      return el('p', 'meta', 'Evento desconhecido');
  }
}
  ...
  if (event.kind === 'tool_call' && event.call.attributionMethod === 'superseded') {
    body.append(
      el(
        'p',
        'meta',
        'Veio de uma conversa já substituída pela compactação: o ChatGPT continuou executando o turno interrompido. Recusado por segurança; não há nada para corrigir.'
      )
    );
  }
...
function paintAgentFilter(): void {
  ...
  buttons.push(chip(null, 'Todos'));
  for (const agent of named) buttons.push(chip(agent, agent));
  if (anyUnattributed) buttons.push(chip(UNATTRIBUTED, 'Sem atribuição'));
...
function compactionState(block: CompactionBlock): { text: string; tone: CompactionTone } {
  ...
  if (abandoned) return { text: `Falhou — ${abandoned.message.text.replace(ABANDONED_NOTE, '')}`, tone: 'bad' };
  const chars = block.handoff ? ` (${compactNumber(block.handoff.chars)} caracteres)` : '';
  if (block.resume) return { text: `Nova conversa aberta às ${clockTime(block.resume.time)}${chars}`, tone: 'good' };
  if (block.handoff) {
    return block.moved
      ? { text: `Resumo salvo${chars}, mas nenhuma conversa nova foi aberta — a conversa continuou aqui`, tone: 'bad' }
      : { text: `Resumo salvo${chars} — abrindo a nova conversa…`, tone: 'wait' };
  }
  if (block.brief?.final) {
    return block.moved
      ? { text: 'Resumo escrito, mas não salvo pelo aplicativo — a conversa continuou aqui', tone: 'bad' }
      : { text: 'Resumo escrito — salvando a continuidade…', tone: 'wait' };
  }
  if (block.brief) return { text: 'O ChatGPT está escrevendo o resumo…', tone: 'wait' };
  return block.moved
    ? { text: 'Nenhum resumo foi escrito — a conversa continuou aqui', tone: 'bad' }
    : { text: 'Resumo solicitado — aguardando o ChatGPT…', tone: 'wait' };
}
...
function compactionRow(block: CompactionBlock): HTMLElement {
  ...
  head.append(el('b', '', 'Compactar e continuar:'));
  head.append(el('span', 'state', state.text));
  box.append(head);

  const raw = el('div', 'raw');
  if (block.prompt) {
    raw.append(el('h4', '', 'Solicitação de resumo'));
  ...
  if (block.brief) {
    raw.append(el('h4', '', block.brief.final ? 'Resumo' : 'Resumo (ainda sendo escrito)'));
    raw.append(renderedMarkdown(block.brief.message.text));
  }
  if (block.handoff) {
    raw.append(
      el('p', 'raw-facts', `Resumo salvo — ${compactNumber(block.handoff.chars)} caracteres (${block.handoff.reason})`)
    );
  }
  if (block.resume) {
    raw.append(
      el(
        'p',
        'raw-facts',
        `Contexto inicial enviado à nova conversa — ${compactNumber(block.resume.message.chars)} caracteres às ${clockTime(block.resume.time)}`
      )
    );
  }
...
function groupToolRows(rows: HTMLElement[], scope = selectedId, groups = toolGroups): HTMLElement[] {
  ...
  for (let i = 0; i < rows.length;) {
    ...
    const label = latestHead?.querySelector('b, span:not(.agent-avatar)')?.textContent || 'Atividade';
    group.querySelector('.activity-title')!.textContent = label;
    group.querySelector('summary')!.title = `${end - i} ações · ${label}`;
...
function paintDetail(): void {
  ...
  $('chatTitle').textContent = summary ? summary.title || 'Conversa sem título' : 'Nova conversa';
  ...
  if (selectedId && historyBefore !== null) {
    ...
    const latest = el('button', 'btn small', 'Voltar ao mais recente');
  ...
  if (!recoveryStatus.hidden && recovery?.kind === 'progress') recoveryStatus.append(icon('i-pulse'), el('span', 'queue-label', recovery.message.text),
    dockAction('Ocultar aviso de recuperação', 'i-x', () => {
  ...
  const facts: string[] = [];
  if (summary) {
    facts.push(`${totalEvents} evento${totalEvents === 1 ? '' : 's'}`);
    if (events.length < totalEvents) facts.push(`mostrando uma página limitada a ${events.length}`);
    if (agentFilter !== null) {
      facts.push(`filtrado por ${agentFilter === UNATTRIBUTED ? 'sem atribuição' : agentFilter} — ${filtered.length} resultados`);
    }
    if (windowed.omitted > 0) facts.push(`${shown.length} mais recentes exibidos`);
    facts.push(`~${compactNumber(summary.contextTokens)} tokens estimados no contexto atual`);
    const level = pressureOf(summary.id);
    if (level && level.level !== 'ok') {
      facts.push(
        level.level === 'huge'
          ? 'limite de compactação excedido — compacte antes de continuar'
          : 'contexto grande — convém compactar em breve'
      );
    }
    if (summary.lastTurnOutcome && summary.lastTurnOutcome !== 'completed') {
      facts.push(`último turno ${TURN_OUTCOME_LABELS[summary.lastTurnOutcome]}`);
    }
  }
...
function paintHandoff(): void {
  ...
  if (handoff) {
    ...
    head.textContent = `${compactNumber(handoff.text.length)} caracteres · de ${handoff.sourceEvents} eventos (~${compactNumber(handoff.sourceTokens)} tokens) · ${ago(handoff.createdAt)}`;
    parts.push(head);
...
function paintStateLine(): void {
  ...
  note.classList.toggle('is-working', text.startsWith('Trabalhando'));
  if (visible && text.startsWith('Trabalhando por ')) durationTimer = window.setTimeout(paintStateLine, 1000);
  repaintBadges();
}
...
function stateLine(): { text: string; tone: '' | 'is-live' | 'is-bad' } {
  if (!deps.state()?.config.ui.developerMode) {
    ...
    if (startedAt === undefined) return { text: active ? 'Trabalhando…' : '', tone: '' };
    if (!active && endedAt === undefined) return { text: '', tone: '' };
    const seconds = Math.max(0, Math.floor(((active ? Date.now() : endedAt!) - startedAt) / 1000));
    return { text: `${active ? 'Trabalhando' : 'Trabalhou'} por ${seconds >= 60 ? `${Math.floor(seconds / 60)}m ` : ''}${seconds % 60}s`, tone: '' };
  }
  ...
  if (selected && selected.conversationId === null) {
    return {
      text: 'Atividade que o aplicativo não conseguiu associar a uma conversa — vinda de outro dispositivo ou sem uma aba do ChatGPT aberta',
      tone: ''
    };
  }
  ...
  const parts: string[] = [];
  if (count('active') > 0) parts.push(`${count('active')} trabalhando`);
  // "invited" is a worker whose ChatGPT tab has been asked for but has not joined yet.
  if (count('invited') > 0) parts.push(`${count('invited')} abrindo`);
  ...
  if (count('detached') > 0) parts.push(`${count('detached')} trabalhando sem aba`);
  if (count('waking') > 0) parts.push(`${count('waking')} retomando`);
  ...
  if (count('sleeping') > 0) parts.push(`${count('sleeping')} em espera`);
  if (count('finished') > 0) parts.push(`${count('finished')} concluídos`);
  if (count('failed') > 0) parts.push(`${count('failed')} falharam`);
  const live = count('invited') + count('active') + count('detached') + count('waking');
  return {
    text: `${workers.length === 1 ? '1 executor' : `${workers.length} executores`} · ${parts.join(' · ')}`,
...
async function showExtensionPath(): Promise<void> {
  ...
  if (dir) {
    node.textContent = `Pasta da extensão: ${dir}`;
    node.classList.remove('is-warn');
  } else {
    node.textContent =
      'A pasta da extensão não foi encontrada nesta instalação. Reinstale o aplicativo ou use a pasta extension/ do código-fonte.';
    ...
    list.replaceChildren(
      el(
        ...
        state.retainedHistory
          ? 'Nenhum executor ativo. Seus históricos reutilizáveis permanecem disponíveis às conversas principais; Limpar grupo de agentes os remove definitivamente.'
          : 'Nenhum agente. O agente principal cria executores pela ação spawn da ferramenta agents.'
      )
    );
  } else {
    list.replaceChildren(
      ...state.agents.map((agent) => {
        ...
        top.append(el('span', `chip is-${agent.state}`, AGENT_BADGE[agent.state].text));
        ...
        if (!over) {
          ...
          clear.title =
            agent.role === 'prime'
              ? 'Limpar sessão — encerrar esta execução e todos os seus executores'
              : `Limpar sessão — encerrar ${agent.id} e liberar sua vaga`;
          top.append(clear);
        }
        const sub = el('div', 'model-sub');
        const bits = [`${agent.pending} pendentes`, `${agent.delivered} entregues`];
        if (agent.conversationId) bits.push('conversa vinculada');
...
function releasedOn(created: number): string {
  if (!created) return 'data de lançamento não informada';
...
async function loadGoalModels(reset: boolean): Promise<void> {
  ...
  $('goalModelsState').textContent = 'Carregando modelos do OpenRouter…';
  ...
  if (!page) {
    ...
    $('goalModelsState').textContent = 'Não foi possível acessar o OpenRouter. O modelo em uso foi mantido.';
...
function paintGoalModels(): void {
  ...
  $('goalModelsState').textContent =
    shown === 0 ? 'Nenhum modelo foi retornado.' : `Mostrando ${shown} de ${goalTotal}, com os lançamentos mais recentes primeiro.`;
...
function applyGoal(state: AppState, previous?: Config): void {
  ...
  automation.title = config.sessions.record ? 'Continuar esta conversa automaticamente' : 'Ative a gravação para usar objetivo ou repetição';
  paintAutomationSwitch();
  ...
  goalKey.placeholder = state.hasGoalKey ? '•••••••• salva' : 'sk-or-v1-…';
  goalKey.disabled = !secureStorageAvailable;
  $('goalKeyState').textContent = !secureStorageAvailable
    ? (state.secureStorage?.detail ?? 'O armazenamento seguro de credenciais está indisponível.')
    : state.hasGoalKey
      ? 'Há uma chave salva no armazenamento seguro do sistema. Digite outra para substituí-la.'
      : 'Salva no armazenamento seguro do sistema. O navegador recebe somente a resposta, nunca a chave.';
...
function wireGoal(save: () => Promise<void>): void {
  ...
  $('goalPromptEdit').addEventListener('click', () => {
    ...
    $('goalPromptEdit').textContent = panel.hidden ? 'Editar instrução' : 'Fechar instrução';
  ...
  $('goalPromptReset').addEventListener('click', async () => {
    ...
    await save();
    toast('Instrução de objetivo sem tarefa restaurada ao padrão');
  });
  ...
  $('goalObjectivePromptEdit').addEventListener('click', () => {
    ...
    panel.hidden = !panel.hidden;
    $('goalObjectivePromptEdit').textContent = panel.hidden ? 'Editar instrução' : 'Fechar instrução';
  ...
  $('goalObjectivePromptReset').addEventListener('click', async () => {
    ...
    await save();
    toast('Instrução de objetivo com tarefa restaurada ao padrão');
  });
  ...
  $('goalLoopPromptEdit').addEventListener('click', () => {
    ...
    panel.hidden = !panel.hidden;
    $('goalLoopPromptEdit').textContent = panel.hidden ? 'Editar instrução' : 'Fechar instrução';
  ...
  $('goalLoopPromptReset').addEventListener('click', async () => {
    ...
    await save();
    toast('Instrução de repetição restaurada ao padrão');
  });
  ...
  $('goalPick').addEventListener('click', () => {
    ...
    panel.hidden = !panel.hidden;
    $('goalPick').textContent = panel.hidden ? 'Selecionar modelo' : 'Fechar';
  ...
  $('goalModelList').addEventListener('click', (event) => {
    ...
    paintGoalModels();
    void save();
    toast(`Modelo do objetivo definido como ${goalModel}`);
  });
  ...
  $('goalKeyRemove').addEventListener('click', async () => {
    ...
    if (next) {
      applyGoal(next);
      toast('Chave OpenRouter removida');
    }
  });
}
...
function applyAutoCompactHint(config: Config): void {
  $('autoCompactHint').textContent = config.compaction.auto
    ? 'Interrompe a resposta ativa ao atingir esse número de tokens, escreve um resumo e abre uma nova conversa.'
    : 'Desativado — somente o botão Compactar e continuar na aba do ChatGPT faz a compactação.';
}
...
export function chatApply(state: AppState, previous?: Config): void {
  ...
  $('bridgeState').textContent = !browserRequired
    ? 'Os recursos que dependem do navegador estão desativados. A extensão não é necessária neste momento.'
    : !secureStorageAvailable
      ? (state.secureStorage?.detail ?? 'O armazenamento seguro de credenciais está indisponível; a extensão não pode conectar com segurança.')
    : !bridge.running
      ? 'A conexão local está desativada, embora a gravação ou os subagentes precisem dela.'
      : bridge.present
        ? `Conectado em 127.0.0.1:${bridge.port ?? '?'} · última mensagem ${ago(bridge.lastSeenAt)}.`
        : bridge.paired
          ? `Autorizada, mas a extensão do navegador não está conectada no momento. ${
              bridge.lastSeenAt === null ? 'Nenhuma comunicação desde que o aplicativo iniciou.' : `Última comunicação ${ago(bridge.lastSeenAt)}.`
            }`
          : `Aguardando em 127.0.0.1:${bridge.port ?? '?'} · nenhum navegador autorizado ou conectado ainda.`;
...
async function refreshInputQueue(): Promise<void> {
  ...
  const reorder = async (from: string, to: string, after: boolean) => {
    ...
    if (saved === false) toast('A fila mudou durante a movimentação. Tente novamente.');
    void refreshInputQueue();
  };
  ...
  reconcileChildren(taskList, queuedTasks.map(entry => {
    ...
    const label = el('span', 'queue-label', entry.text); label.title = `${entry.state === 'queued' ? (entry.mode === 'after-turn' ? 'Após a próxima resposta concluída' : 'Ao encerrar a sessão ou após uma resposta concluída') : 'Aguardando confirmação'} · ${entry.text}`;
    ...
    if (entry.state === 'queued') {
      ...
      if (entry.mode === 'finish' && selection?.conversationId === queueSessionSummary?.conversationId && isAstraModel(selection?.model, selection?.reasoningEffort)) {
        const delivery = el('button', 'btn queue-delivery', entry.afterTurn === true ? 'Também após o turno' : 'Somente no encerramento') as HTMLButtonElement;
        delivery.type = 'button';
        delivery.setAttribute('aria-label', 'Enviar também esta tarefa como nova mensagem após o Astra concluir o turno');
        delivery.setAttribute('aria-pressed', String(entry.afterTurn === true));
        delivery.title = entry.afterTurn === true ? 'No encerramento da sessão ou como nova mensagem após a conclusão confirmada do turno' : 'Somente no resultado da ferramenta de encerramento; não iniciar um novo turno';
      ...
      label.tabIndex = 0;
      label.title = 'Arraste para reordenar. Alt + seta para cima/baixo também move esta tarefa.';
      ...
      const edit = dockAction('Editar tarefa da fila', 'i-pencil', () => {});
      edit.onclick = () => {
        const field = document.createElement('textarea'); field.value = entry.text; field.maxLength = 16000; field.setAttribute('aria-label', 'Tarefa da fila');
        const save = el('button', 'btn', 'Salvar'); save.setAttribute('type', 'button');
      ...
      const cancel = dockAction('Remover tarefa da fila', 'i-trash', () => {}); cancel.onclick = async () => { await run(api.cancelInput(entry.id)); void refreshInputQueue(); };
  ...
  $('inputQueue').replaceChildren(...rows.map((entry) => {
    ...
    const status = entry.error || (entry.state === 'failed' ? 'Entrega não confirmada' : entry.state === 'decision' ? 'Preparando continuação' : entry.state === 'browser' ? 'Confirmação de entrega pendente' : entry.state === 'tool' ? 'Enviado ao turno ativo · aguardando confirmação' : entry.dueAt > Date.now() ? `Agendado para ${new Date(entry.dueAt).toLocaleString()}` : 'Na fila');
    ...
    row.append(receipt);
    if (notice(entry)) {
      row.append(dockAction('Ocultar aviso de entrega', 'i-x', () => dismissInputNotice(entry.id)));
      const retry = dockAction('Tentar enviar novamente', 'i-retry', () => {});
      retry.classList.add('delivery-retry');
      retry.title = 'Restaurar esta mensagem no editor para revisão e envio';
      retry.onclick = () => {
        const input = $<HTMLTextAreaElement>('chatInput');
        if (input.value.trim() || imageDrafts.get(draftKey())?.length) { toast('Envie ou limpe o rascunho atual antes de tentar enviar esta mensagem novamente.'); return; }
    ...
    if (['queued', 'browser'].includes(entry.state)) {
      const cancel = dockAction('Cancelar envio', 'i-x', () => {});
    ...
    if (entry.state === 'queued' && entry.error?.startsWith('Mensagem na fila. Falha ao iniciar o navegador:')) {
      const retry = dockAction('Tentar abrir o navegador novamente', 'i-retry', () => {});
      retry.classList.add('delivery-retry');
  ...
  for (const helper of pausedHelpers ?? []) {
    ...
    row.append(el('span', '', 'A entrega ao auxiliar não foi confirmada. A conversa anterior ainda pode estar em execução.'));
    const retry = el('button', 'btn', 'Iniciar um novo auxiliar');
    retry.setAttribute('type', 'button');
    retry.onclick = async () => {
      retry.setAttribute('disabled', '');
      const accepted = await run(api.retryHelper(helper.id, helper.sourceSessionId));
      if (accepted) toast('Novo auxiliar autorizado para esta conversa');
      else toast('Este auxiliar mudou. Atualizando seu estado.');
      void refreshInputQueue();
    };
    row.append(retry);
...
async function sendComposer(delivery?: 'finish', plan?: string[]): Promise<boolean | void> {
  ...
  const images = imageDrafts.get(key) ?? [];
  const text = plan?.[0] ?? (input.value.trim() || (images.length ? 'Analise as imagens anexadas, por favor.' : ''));
  ...
  if (!modelSettings) { toast('Atualize os modelos e selecione um modelo disponível e seu esforço antes de enviar.'); return false; }
  const sessionId = selectedId;
...
function selectNewChat(projectId: string | null = null): void {
  ...
  $<HTMLTextAreaElement>('chatInput').placeholder = projectId ? `Mensagem em ${projects.find(project => project.id === projectId)?.name ?? 'projeto'}…` : 'Escreva sua mensagem…';
...
export function initChat(next: Deps): void {
  ...
  agentToggle.setAttribute('aria-label', 'Mostrar ou ocultar painel lateral dos subagentes'); agentToggle.setAttribute('aria-expanded', 'false');
  ...
  for (const buttonId of ['saveSessionObjective'] as const) {
    $(buttonId).addEventListener('click', async () => {
      ...
      if (!id) {
        ...
        if (!settings) { toast('Atualize os modelos e selecione um modelo disponível e seu esforço antes de enviar.'); return; }
        ...
        try {
          ...
          if (!opening) { if (goalProgress?.requestId === requestId) { goalProgress.phase = 'failed'; goalProgress.error ||= 'Falha ao gerar a mensagem inicial'; paintGoalProgress(); } return; }
          ...
          try {
            ...
            } else if (goalProgress?.requestId === requestId) { goalProgress.phase = 'failed'; goalProgress.error = 'Não foi possível colocar a mensagem inicial na fila'; }
      ...
      try {
        ...
        if (goalProgress?.requestId === requestId) { goalProgress.phase = saved ? 'saved' : 'failed'; if (!saved) goalProgress.error = 'Não foi possível salvar a tarefa'; paintGoalProgress(); }
  ...
  const appendImages = (key: string, chosen: InputImage[] | null | undefined): void => {
    ...
    if (combined.length > 4) { toast('Anexe até quatro imagens por mensagem'); return; }
  ...
  $('composer').addEventListener('drop', async event => {
    ...
    if (files.length + (imageDrafts.get(key)?.length ?? 0) > 4) { toast('Anexe até quatro imagens por mensagem'); return; }
  ...
  $('copyHandoff').addEventListener('click', async () => {
    ...
    if (copied) toast('Resumo de continuidade copiado');
  });
  ...
  $('swarmList').addEventListener('click', async (event) => {
    ...
    toast(
      outcome.cleared === 'run'
        ? 'Execução limpa — todos os executores foram encerrados'
        : outcome.cleared === 'worker'
          ? `${id} encerrado — vaga liberada`
          : outcome.reason
    );
  });

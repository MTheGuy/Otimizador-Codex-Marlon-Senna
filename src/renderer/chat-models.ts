...
const effortNames: Record<string, string> = { none: 'Instantâneo', minimal: 'Mínimo', low: 'Baixo', medium: 'Médio', high: 'Alto', xhigh: 'Extra alto', max: 'Máximo', ultra: 'Ultra', pro: 'Pro' } satisfies Record<ReasoningEffort, string>;
...
function options(select: HTMLSelectElement, choices: Array<{ id: string; label: string }>, value: string): void {
  ...
  if (!desired.length && !value) {
    const unavailable = option('Nenhuma opção encontrada', ''); unavailable.disabled = true; desired.push(unavailable);
  }
  if (value && !choices.some(choice => choice.id === value)) {
    const unverified = option(`${value} · não verificado`, value);
...
function paintComposerChoices(): void {
  ...
  if (!steps.length) {
    if (title) title.textContent = 'Nenhuma opção disponível';
    if (subtitle) subtitle.textContent = 'Recarregar modelos';
    return;
  }
  ...
  slider.setAttribute('aria-label', 'Modelo e esforço de raciocínio');
  const show = () => {
  ...
  if (current >= 0) show();
    ...
    if (title) title.textContent = 'Escolha um nível';
    if (subtitle) subtitle.textContent = 'Seleção anterior indisponível';
    slider.setAttribute('aria-valuetext', 'Escolha um modelo e esforço disponíveis');
  }
  const choose = () => {
...
function paintComposerLabel(): void {
  ...
  const label = confirmed
    ...
    : catalog.state === 'pending' ? 'Carregando modelos…' : 'Selecionar modelo';
...
function paintStatus(): void {
  ...
  const message = catalog.state === 'pending' ? 'Consultando os modelos da sua conta…'
    : catalog.state === 'ready' ? `Disponível na sua conta ChatGPT · verificado às ${new Date(catalog.observedAt!).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
    : catalog.error ?? 'Conecte-se ao ChatGPT para carregar seus modelos.';
  for (const id of ['chatModelStatus', 'composerModelStatus']) {
    const node = document.getElementById(id);
    if (node) {
      node.textContent = id === 'composerModelStatus' ? (catalog.state === 'pending' ? 'Carregando modelos…' : catalog.error ?? 'Modelos indisponíveis · tentar consultar novamente') : message;
  ...
  for (const id of ['refreshChatModels', 'refreshComposerModels']) {
    ...
    if (button) {
      ...
      if (id === 'refreshComposerModels') {
        button.hidden = false;
        button.title = catalog.state === 'pending' ? 'Consultando modelos do ChatGPT' : 'Recarregar modelos do ChatGPT';

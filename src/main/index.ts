...
if (process.platform === 'win32') app.setAppUserModelId(BRANDING.appId);
...
function refreshTray(): void {
  ...
  const label = connected ? 'Conectado' : offline ? 'Sem internet' : 'Desconectado';
  tray.setImage(trayIcon(running));
  tray.setToolTip(`${BRANDING.name} — ${label.toLowerCase()}`);
  tray.setContextMenu(
    Menu.buildFromTemplate([
      { label, enabled: false },
      { type: 'separator' },
      { label: 'Abrir', click: windowActivation.request },
      {
        label: running ? 'Desconectar' : 'Conectar',
  ...
  setBrowserOpener(async (url) => {
    ...
    logWarn(
      'Chrome/Chromium was not found for a browser-backed worker/resume command; falling back to the default browser. ' +
        'Se a extensão do aplicativo não estiver nesse navegador, abra o endereço do ChatGPT no Chrome.'
    );

...
export class IdentityLostError extends AgentError {
  constructor() {
    super(
      'WORKER_IDENTITY_LOST: O aplicativo não identificou de qual conversa veio esta chamada, portanto não pode ' +
        'agir sobre a execução daqui. Nenhuma operação de agente foi realizada. Aguarde um pouco e tente esta operação mais uma vez. ' +
        'Se a recusa persistir, preserve seu resultado na conversa e informe que a entrega não foi confirmada; ' +
        'não afirme que a mensagem ou o relatório de conclusão chegou ao destinatário.'
    );
  }
}
...
function requireEnabled(): void {
  if (!getConfig().multiAgent.enabled) {
    throw new AgentError('O modo multiagente está desativado no aplicativo. Solicite ao usuário que o habilite.');
  }
}
...
function planRevivalText(agent: Agent): { text: string; messageIds: string[] } {
  ...
  const text =
    ...
    `\n\n(Local application: you are still ${agent.info.id} in the same run, and this is the prime agent talking to ` +

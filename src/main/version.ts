...
export function extensionDownloadUrl(version = APP_VERSION): string {
  const repository = BRANDING.publication.repository;
  if (!repository) return '';
  return `https://github.com/${repository}/releases/download/v${encodeURIComponent(version)}/${BRANDING.installerPrefix}-Extension.zip`;
}
 ...
 * 7 — the application identity changed, and the `app` field every bridge response

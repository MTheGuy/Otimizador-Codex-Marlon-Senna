...
export function describeMacOSDesktopAccess(
  ...
  if (caps.control) {
    checks.push(check(
      ...
      'O macOS negou acesso AXUIElement ao componente nativo do aplicativo. Abra Privacidade e Segurança → Acessibilidade (Controle de dispositivo e acesso a dados em versões mais recentes do macOS).'
    ));
  }
  return checks;
}

...
export function redact(message: string): string {
  return message
    // Credenciais curtas continuam sensíveis; cabeçalhos podem carregar vários cookies na mesma linha.
    .replace(/(\b(?:proxy-authorization|authorization|set-cookie|cookie)["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\r\n]*)/gi, '$1***')
    .replace(/(\b(?:[\w-]{0,64}(?:api[-_]?key|token|secret)|password|passwd)["']?\s*[:=]\s*)(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;&\]}]+)/gi, '$1***')

...
const LINUX_STORAGE_PROBE = 'local-agents-safe-storage-probe';
    ...
    if (platform === 'linux') {
      ...
      if (!secureStorageCiphertextIsProtected(probe, platform)) {
        return {
          available: false,
          detail:
            'O armazenamento seguro do Linux recorreu ao provedor inseguro do Electron com chave fixa. Inicie ou desbloqueie um gerenciador de credenciais/Secret Service, como GNOME Keyring ou KWallet, e reinicie o aplicativo.'
        };
      }
    }

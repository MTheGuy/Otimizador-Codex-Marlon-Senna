 ...
 *
 * Instalações novas começam em STRICT. Recursos permanecem disponíveis, mas acesso ao
 * computador depende das permissões escolhidas; ausência de configuração nunca é consentimento.
 */
/**
...
// Workers continuam disponíveis, mas nenhuma chamada ganha autoridade por falta de identidade.
...
const configSchema = z.object({
  ...
  readOnly: z.boolean(),
  security: z.object({ reviewed: z.boolean().optional().default(false) })
    .optional().default({ reviewed: false }),
  tunnel: z.object({
  ...
  ui: z.object({
    displayName: z.string().trim().max(120).optional().default(BRANDING.name)
      .catch(BRANDING.name).transform(value => value || BRANDING.name),
  ...
  multiAgent: z
    ...
    defaultReasoning: z.enum(['', ...REASONING_EFFORTS]).optional(),
      ...
      allowUnattributedCalls: z.boolean().optional().default(false).transform(() => false),
...
/** STRICT vale em todos os sistemas; a plataforma pode restringir, nunca ampliar, esse padrão. */
...
export function defaultConfig(platform: NodeJS.Platform = process.platform, release?: string): Config {
  return {
    ...
    ui: { displayName: BRANDING.name, minimizeToTray: true, autoConnect: false, privacyScreenshots: false, theme: 'dark' },
 ...
 *
 * Configuração ausente ou corrompida nunca autoriza escrita, comandos ou controle do computador.
 * Na recuperação, automações que dependiam do estado perdido também permanecem desativadas.
 */

  ...
  return {
    ...
    guarded(cap, name, fn) {
      return guard(name, async () => {
        if (!caps[cap]) {
          return fail(
            `TOOL_DISABLED: ${name} está desativado nas permissões atuais do aplicativo. ` +
              'Solicite ao usuário que habilite essa permissão no aplicativo e tente novamente. Se a lista de ferramentas estiver desatualizada, inicie uma nova conversa.'
          );
        }
        return fn();
      });
    ...
    featureDisabled(feature, setting) {
      return fail(
        `FEATURE_DISABLED: ${feature} está desativado no aplicativo. ` +
          `Solicite ao usuário que habilite "${setting}" no aplicativo e tente novamente.`
      );
    }
  };
}

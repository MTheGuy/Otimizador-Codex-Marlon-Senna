      ...
      async ({ paths, start_line, end_line, max_bytes }) =>
        guard('read', async () => {
          if (!caps.read && !caps.browse && !caps.metadata) {
            return fail(
              'TOOL_DISABLED: read está desativado nas permissões atuais. Solicite ao usuário que habilite a leitura no aplicativo.'
            );
          }
    ...
    reg.register(
      ...
      async ({ path }) =>
        guard('view_image', async () => {
          if (!caps.read) {
            return fail(
              'TOOL_DISABLED: view_image está desativado nas permissões atuais. Solicite ao usuário que habilite a leitura no aplicativo.'
            );
          }
      ...
      async ({ patch }) =>
        guard('apply_patch', async () => {
          if (!caps.create && !caps.edit && !caps.move && !caps.deleteFile) {
            return fail(
              'TOOL_DISABLED: apply_patch está desativado nas permissões atuais. Solicite ao usuário que habilite a alteração de arquivos no aplicativo.'
            );
          }
      ...
      async (input) =>
        reg.guarded('command', 'write_stdin', async () => {
          ...
          if (execOwnershipDenied(input.session_id, asking)) {
            return fail(
              `write_stdin falhou: não há prova de que a sessão ${input.session_id} pertence a esta conversa persistente. Inicie sua própria sessão com exec_command ou tente novamente após a extensão reconectar.`
            );
          }

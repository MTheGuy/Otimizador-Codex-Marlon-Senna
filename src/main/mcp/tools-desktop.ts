      ...
      async ({ actions, frameId, verify, captureAfter, captureWindow, captureFull, captureMaxWidth, captureCrop }) =>
        guard('computer', async () => {
          ...
          if (!caps.control && actions.some((a) => a.type !== 'wait' && !a.type.endsWith('_clipboard'))) {
            return fail(
              'TOOL_DISABLED: o controle de mouse e teclado está desativado nas permissões atuais. ' +
                'Solicite ao usuário que habilite "Controlar mouse e teclado" no aplicativo e tente novamente.'
            );
          }
          const parsed: Action[] = [];

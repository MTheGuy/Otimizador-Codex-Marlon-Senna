...
const settingsPatch = z.object({
  ...
  readOnly: z.boolean(),
  security: z.object({ reviewed: z.boolean() }).optional(),
  tunnel: z.object({
...
function mergeSettings(current: Config, base: SettingsSnapshot, wanted: SettingsSnapshot): SettingsSnapshot {
  ...
  return {
    ...
    security: {
      reviewed: pick(current.security?.reviewed ?? false, base.security?.reviewed ?? false, wanted.security?.reviewed ?? false)
    },
    tunnel: {
    ...
    ui: {
      displayName: pick(current.ui.displayName, base.ui.displayName, wanted.ui.displayName),
...
let ipcWindow: (() => BrowserWindow | null) | null = null;

/** Somente a janela principal pode solicitar ações locais; iframes não recebem essa autoridade. */
function handle<T>(channel: string, fn: (payload: unknown) => Promise<T>): void {
  ipcMain.handle(channel, async (event, payload: unknown) => {
    try {
      const owner = ipcWindow?.();
      if (!owner || owner.isDestroyed() || event?.sender !== owner.webContents ||
          !event.senderFrame || event.senderFrame !== owner.webContents.mainFrame) {
        throw new Error('Solicitação recusada: janela de origem não autorizada.');
      }
...
export function registerIpc(getWindow: () => BrowserWindow | null): void {
  ...
  handle('roots:rename', async (payload) => {
    ...
    if (RESERVED_ROOT_NAMES.has(newName)) {
      throw new SandboxError(`/${newName} é reservado pelo aplicativo e não pode ser usado como nome de pasta`);
    }
    await updateConfig((config) => {
      if (!config.roots.some((root) => root.name === name)) throw new Error(`/${name} is not an approved folder`);
  ...
  handle('bridge:downloadExtension', async () => {
    ...
    const url = extensionDownloadUrl(app.getVersion());
    if (!url) throw new Error('Nenhum endereço de distribuição foi configurado. Use a extensão incluída no instalador.');
    await shell.openExternal(url);
    return true;
  });

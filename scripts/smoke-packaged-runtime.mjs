...
const branding = readConfiguration(repository);
...
let appExecutable;
if (targetPlatform === 'darwin') {
  const appBundle = path.join(packageRoot, `${branding.name}.app`);
  resourcesDir = path.join(appBundle, 'Contents', 'Resources');
  appExecutable = path.join(appBundle, 'Contents', 'MacOS', branding.executableName);
} else {
  resourcesDir = path.join(packageRoot, 'resources');
  appExecutable = path.join(packageRoot, targetPlatform === 'win32' ? `${branding.executableName}.exe` : branding.packageName);
}

...
import { projectPath } from './customize-config.mjs';
...
for (const directory of ['build', 'out', 'release', 'resources', 'node_modules']) projectPath(root, directory);
const cache = projectPath(root, 'resources/packaging/cache');
const temporary = projectPath(root, 'resources/packaging/cache/tmp');
...
run(node, ['scripts/customize.mjs', '--apply']);

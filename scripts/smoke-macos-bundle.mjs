...
import { readConfiguration } from './customize-config.mjs';
import {
...
const app = process.argv[3]
  ...
  : path.join(releaseDir, unpackedDir, `${branding.name}.app`);
...
const expectedPlist = {
  ...
  NSScreenCaptureUsageDescription:
    `${branding.name} captura uma tela ou janela somente quando o conector Desktop habilitado solicita observá-la.`
};
...
const mainExecutable = path.join(contents, 'MacOS', branding.executableName);

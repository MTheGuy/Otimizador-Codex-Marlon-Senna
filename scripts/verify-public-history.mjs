...
const publication = JSON.parse(readFileSync(new URL('../customization/app.config.json', import.meta.url), 'utf8')).publication?.repository ?? '';
const maintainerLogin = publication.split('/')[0].toLowerCase();
const safeMaintainerEmail = /^(?:\d+\+)?[a-z0-9-]+@users\.noreply\.github\.com$/i;
...
const blockedText = [
  ...
  { label: 'private user path', value: homedir() },
];
...
function checkMaintainerIdentity(name, email, location) {
  ...
  const belongsToMaintainer =
    maintainerLogin && (normalizedName === maintainerLogin || normalizedEmail.includes(maintainerLogin));
...
function publishedCommits() {
  ...
  const canonical = remotes.find((remote) => {
    ...
    const repository = url.replace(/^(?:https?:\/\/github\.com\/|ssh:\/\/git@github\.com\/|git@github\.com:)/i, '').replace(/(?:\.git)?\/?$/, '');
    return Boolean(publication) && repository.toLowerCase() === publication.toLowerCase();
  });
